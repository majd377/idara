...
async function navigate(view,periodId=null,force=false){if(!state.profile||state.profile.role==='pending'){renderPending();return;}if(state.profile.role==='resident'){renderResidentHome();$('#sidebar')?.classList.remove('open');return;}state.view=view;if(periodId)state.periodId=periodId;setActiveNav();await loadData(force);render();$('#sidebar')?.classList.remove('open');}
...
function subscriberFinancialSeries(id){
  ...
  return periods.map(p=>{
    const priorDebt=Math.max(0,runningSigned),priorCredit=Math.max(0,-runningSigned);
    ...
    const previousDebt=priorDebt+explicitOpeningDebt;
    const before=Math.max(0,previousDebt+currentCharges-priorCredit);
    const signedAfter=before-periodPayments;
    const finalBalance=Math.max(0,signedAfter),creditCarry=Math.max(0,-signedAfter);
    const result={
      s,p,previousBalance:runningSigned,previousDebt,previousCredit:priorCredit,explicitOpeningDebt,
      ...
      finalBeforePayments:before,finalBalance,creditCarry,appliedPrice:water.price||0
    };
    runningSigned=signedAfter;
    return result;
  });
}
function subscriberFinancialSummary(id,periodId=null){
  ...
  if(!series.length){
    ...
    return {s,p,previousBalance:b,previousDebt:Math.max(0,b),previousCredit:Math.max(0,-b),currentConsumption:0,currentWater:0,guard:0,services:0,other:0,periodPayments:0,totalPaymentsAll:0,finalBeforePayments:Math.max(0,b),finalBalance:Math.max(0,b),creditCarry:Math.max(0,-b),appliedPrice:0};
  }
...
async function downloadResidentPdf(id,weekId,fromId,toId){
  ...
  const tableWrap=source.querySelector('.resident-ledger-table');if(!tableWrap){toast('تعذر العثور على جدول الكشف','error');return;}
  const clone=tableWrap.cloneNode(true);
  clone.style.cssText='display:block;overflow:visible;width:1200px;max-width:none;background:#fff;';
  clone.querySelectorAll('*').forEach(el=>{el.style.maxWidth='none';});
  clone.querySelectorAll('table').forEach(t=>{t.style.width='100%';t.style.minWidth='0';table=>{}});
  clone.querySelectorAll('table').forEach(t=>{t.style.borderCollapse='collapse';});
  const host=document.createElement('div');host.dir='rtl';
  host.style.cssText='position:fixed;left:-20000px;top:0;width:1240px;background:#fff;padding:28px;direction:rtl;font-family:Arial,sans-serif;color:#183734;z-index:-1;';
  const sum=subscriberFinancialSummary(id,weekId);
  const title=document.createElement('div');
  title.innerHTML=`<div style="font-size:24px;font-weight:700;margin-bottom:8px;">كشف حساب الساكن</div>
    <div style="font-size:18px;font-weight:700;margin-bottom:4px;">${safe(sum.s?.name||'الساكن')} — ${safe(subscriberRow(sum.s).unitCode||'')}</div>
    <div style="font-size:13px;margin-bottom:18px;">من ${safe(fmtDate(periodById(fromId)?.startDate)||'الأقدم')} إلى ${safe(fmtDate(periodById(toId)?.endDate)||'الأحدث')}</div>`;
  host.appendChild(title);host.appendChild(clone);document.body.appendChild(host);
  await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
  const filename=`كشف_حساب_${String(sum.s?.name||'ساكن').replace(/[\\/:*?"<>|]+/g,'_')}.pdf`;
  try{
    await html2pdf().set({
      margin:6,filename,image:{type:'jpeg',quality:.98},
...
onAuthStateChanged(auth,async user=>{state.user=user;$('#boot')?.classList.add('hidden');if(!user){$('#auth-screen')?.classList.remove('hidden');$('#app-shell')?.classList.add('hidden');return;}$('#auth-screen')?.classList.add('hidden');$('#app-shell')?.classList.remove('hidden');$('#userName').textContent=user.displayName||user.email||'المستخدم';$('#userAvatar').textContent=(user.displayName||user.email||'م').slice(0,1);$('#userRole').textContent='جارٍ التحقق…';try{state.profile=await ensureProfile();$('#userRole').textContent=roleName(state.profile.role);if(state.profile.role==='pending'){renderPending();return;}if(state.profile.role==='resident'){renderResidentHome?.();return;}await loadData(true);await ensureDefaults();await navigate('dashboard');}catch(e){console.error(e);const code=e?.code||'';const msg=code==='permission-denied'?'تم تسجيل الدخول، لكن هذا الحساب لا يملك صلاحية الوصول إلى بيانات النظام. من حساب المدير، امنحه دورًا مثل محاسب أو مشاهد إذا كان من المفترض أن يستخدم لوحة الإدارة.':(e?.message||'تعذر تحميل البيانات.');$('#app').innerHTML=`<section class="panel" style="max-width:820px;margin:40px auto"><h2>تعذر تحميل البيانات</h2><p class="muted">${safe(msg)}</p><p class="muted" style="margin-top:10px">رمز الخطأ: ${safe(code||'غير متوفر')}</p></section>`;}});

function renderResidentHome(){
  setActiveNav?.();
  const name=safe(state.user?.displayName||state.user?.email||'المستخدم');
  $('#page-title').textContent='حسابي';
  $('#page-subtitle').textContent='الوصول الخاص بالمستخدم';
  $('#crumbText').textContent='حسابي';
  $('#app').innerHTML=`<section class="hero"><div><span class="eyebrow">بوابة الساكن</span><h2>مرحبًا ${name}</h2><p class="muted">هذا الحساب بصلاحية ساكن، لذلك لا يمكنه فتح لوحة الإدارة أو تعديل بيانات العمارة.</p></div></section><section class="panel" style="margin-top:18px"><div class="panel-head"><div><h2>صلاحية الحساب</h2><p>يمكن للمدير تغيير الصلاحية من الإعدادات.</p></div></div><div class="empty"><b>الحساب يعمل بشكل صحيح.</b><span>إذا كان هذا المستخدم موظفًا في الإدارة، يجب أن يمنحه المدير دورًا مناسبًا مثل محاسب أو موظف قراءات أو مشاهد.</span></div></section>`;
}
