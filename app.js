...
const COLLECTIONS=['buildings','units','subscribers','meters','periods','readings','sources','energyReadings','costs','payments','ledger','members','waterSummary','seedDeletes','debts'];
...
function render(){const fn={dashboard:renderDashboard,periods:renderPeriods,readings:renderReadings,energy:renderEnergy,costs:renderCosts,subscribers:renderSubscribers,payments:renderPayments,debts:renderDebts,reports:renderReports,settings:renderSettings,guide:showGuide,historical:renderHistorical}[state.view]||renderDashboard;fn();}
...
function messageForResident(sum){
  ...
  const total=Math.max(0,sum.finalBalance);
