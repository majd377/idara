...
function currentTotals(pid){
  ...
  const expenseTotal=cs.filter(c=>c.direction!=='credit').reduce((a,c)=>a+num(c.amount),0);
  const contributionTotal=(state.data.contributions||[]).filter(c=>c.periodId===pid).reduce((a,c)=>a+num(c.amount),0);
  const extras=expenseTotal-contributionTotal;
...
function readingTable(pid){
  ...
  const p=periodById(pid);return `<div class="table-wrap"><table class="table" style="min-width:980px"><thead><tr><th>الكود</th><th>الساكن</th><th>البناية / النوع</th><th>القراءة السابقة</th><th>القراءة الحالية</th><th>السحب</th><th>سعر الكوب</th><th>القيمة</th><th>الحالة</th><th>إجراء</th></tr></thead><tbody>${rows.length?rows.map(x=>{const {s,r}=x;const cons=r.currentReading!=null&&r.previousReading!=null?Math.max(0,num(r.currentReading)-num(r.previousReading))/1000:null;const price=r.unitPrice??p.waterUnitPrice??'';const charge=cons!=null&&price!==''?cons*num(price):null;return `<tr data-rid="${r.id}"><td><span class="code">${safe(s.code)}</span></td><td><b>${safe(s.name)}</b></td><td>${s.type==='خارجي'?'<span class="badge warn">خارجي</span>':safe(subscriberRow(s).buildingName)}</td><td><input class="reading-input prev" type="number" step="0.001" value="${r.previousReading??''}"></td><td><input class="reading-input current" type="number" step="0.001" value="${r.currentReading??''}"></td><td class="cons">${cons==null?'—':fmt(cons,3)}</td><td><input class="reading-input price" type="number" step="0.01" value="${price}"></td><td class="charge">${charge==null?'—':money(charge)}</td><td class="status">${r.currentReading==null?'<span class="badge warn">بانتظار</span>':((num(r.currentReading)<num(r.previousReading))?'<span class="badge danger">تحقق</span>':'<span class="badge ok">جاهزة</span>')}</td><td><button class="mini red" data-delete-reading="${r.id}">حذف</button></td></tr>`}).join(''):`<tr><td colspan="10">${empty('لا توجد قراءات','تأكد من وجود سكان وعدادات قبل فتح الأسبوع.')}</td></tr>`}</tbody></table></div><div class="readings-actions"><span class="muted">حفظ القراءة لا يعتمد على حساب السعر؛ الحساب النهائي يتم من صفحة «الأسابيع والحساب».</span><button class="btn primary" id="saveReadings">حفظ كل القراءات</button></div>`;
}
...
async function deleteReading(id){
  if(!can('admin','manager','accountant','operator')){toast('حذف القراءة غير مسموح','error');return;}
  if(!confirm('حذف قراءة المياه هذه؟'))return;
  await deleteDoc(orgDoc('readings',id));removeLocal('readings',id);toast('تم حذف القراءة');renderReadings();
}
async function deleteEnergyReading(id){
  if(!can('admin','manager','accountant')){toast('حذف قراءة الكهرباء مخصص للإدارة','error');return;}
  if(!confirm('حذف قراءة الكهرباء هذه؟'))return;
  await deleteDoc(orgDoc('energyReadings',id));removeLocal('energyReadings',id);toast('تم حذف قراءة الكهرباء');renderEnergy();
}


async function deleteBuilding(id){
  if(!can('admin')){toast('حذف البناية مخصص للمدير','error');return;}
  const b=(state.data.buildings||[]).find(x=>x.id===id);if(!b)return;
  if((state.data.units||[]).some(x=>x.buildingId===id)){toast('لا يمكن حذف البناية وفيها وحدات. احذف/انقل الوحدات أولًا.','error');return;}
  if(!confirm(`حذف ${b.name}؟`))return;
  await deleteDoc(orgDoc('buildings',id));removeLocal('buildings',id);toast('تم حذف البناية');renderSubscribers();
}
async function deleteSource(id){
  if(!can('admin')){toast('حذف المصدر مخصص للمدير','error');return;}
  const src=(state.data.sources||[]).find(x=>x.id===id);if(!src)return;
  if((state.data.energyReadings||[]).some(x=>x.sourceId===id)){toast('لا يمكن حذف مصدر له قراءات محفوظة. أوقفه بدل الحذف.','error');return;}
  if(!confirm(`حذف مصدر ${src.name}؟`))return;
  await deleteDoc(orgDoc('sources',id));removeLocal('sources',id);toast('تم حذف المصدر');renderEnergy();
}
function renderEnergy(){
  ...
  $('#app').innerHTML=`<section class="panel"><div class="panel-head"><div><h2>كهرباء المولدات</h2><p>القراءة الحالية − السابقة = الكيلو المستهلك، ثم × سعر الكيلو = التكلفة.</p></div><div class="panel-actions"><button class="btn soft" id="sourceManage">+ إضافة مصدر/مولد</button><button class="btn primary" id="energySave">حفظ القراءات</button></div></div>${!p?`<div class="reading-start"><div class="reading-start-icon">ϟ</div><div><h2>افتح أسبوعًا أولًا</h2><p>بعد فتحه ستظهر لك كل المولدات.</p><button class="btn primary" id="energyFirst">+ افتح أسبوعًا</button></div></div>`:`<div class="period-picker"><label>الأسبوع الحالي</label><select id="periodSelect" class="period-select">${periods.map(x=>`<option value="${x.id}" ${x.id===p.id?'selected':''}>${safe(x.label||'أسبوع')} — ${fmtDate(x.startDate)} إلى ${fmtDate(x.endDate)} — ${statusText(x.status||'Draft')}</option>`).join('')}</select></div><div class="section-note"><b>المعادلة:</b> (القراءة الحالية − السابقة) × سعر الكيلو = تكلفة المصدر. بعد الحفظ تظهر التكلفة تلقائيًا في حساب الأسبوع.</div><div class="table-wrap"><table class="table" style="min-width:900px"><thead><tr><th>المصدر</th><th>القراءة السابقة</th><th>القراءة الحالية</th><th>الاستهلاك</th><th>سعر الكيلو</th><th>التكلفة</th><th>الحالة</th><th>إجراء</th></tr></thead><tbody>${ers.length?ers.map(r=>{const src=sources.find(s=>s.id===r.sourceId);const cons=r.currentReading!=null&&r.previousReading!=null?Math.max(0,num(r.currentReading)-num(r.previousReading)):null;const cost=cons!=null&&r.pricePerKwh!=null?cons*num(r.pricePerKwh):null;return `<tr data-eid="${r.id}"><td><b>${safe(src?.name||'مصدر محذوف')}</b></td><td><input class="reading-input eprev" type="number" step="0.001" value="${r.previousReading??''}"></td><td><input class="reading-input ecur" type="number" step="0.001" value="${r.currentReading??''}"></td><td class="econs">${cons==null?'—':fmt(cons,3)}</td><td><input class="reading-input erate" type="number" step="0.01" value="${r.pricePerKwh??''}"></td><td class="ecost">${cost==null?'—':money(cost)}</td><td class="estatus">${r.currentReading==null?'<span class="badge warn">بانتظار</span>':(num(r.currentReading)<num(r.previousReading)?'<span class="badge danger">تحقق</span>':'<span class="badge ok">جاهز</span>')}</td><td><button class="mini red" data-delete-energy="${r.id}">حذف</button></td></tr>`}).join(''):`<tr><td colspan="8">${empty('لا توجد مصادر طاقة','أضف مولدًا أو مصدرًا ثم أنشئ أسبوعًا جديدًا.')}</td></tr>`}</tbody></table></div><div class="panel" style="margin-top:13px;background:#fbfcfb"><div class="panel-head"><div><h2>مصادر الطاقة</h2><p>هذه الأسماء ستظهر تلقائيًا في كل أسبوع جديد.</p></div></div><div class="members">${sources.map(src=>`<div class="member-row"><div class="avatar">ϟ</div><div class="member-info"><b>${safe(src.name)}</b><span>${safe(src.type||'مولد')} • ${src.defaultRate!=null?`السعر الافتراضي ${money(src.defaultRate)}`:'بدون سعر افتراضي'}</span></div><span class="badge ${src.active!==false?'ok':'warn'}">${src.active!==false?'فعال':'موقوف'}</span><button class="mini" data-edit-source="${src.id}">تعديل</button><button class="mini red" data-delete-source="${src.id}">حذف</button></div>`).join('')||empty('لا توجد مصادر','أضف أبو زايد أو السويسي أو مولدًا خارجيًا.')}</div></div>`}</section>`;
  $('#sourceManage').onclick=showSourceForm;$$('[data-edit-source]').forEach(b=>b.onclick=()=>showSourceForm(b.dataset.editSource));$$('[data-delete-source]').forEach(b=>b.onclick=()=>deleteSource(b.dataset.deleteSource));$('#energySave').onclick=()=>p?saveEnergy(p.id):null;if($('#energyFirst'))$('#energyFirst').onclick=showPeriodForm;$('#periodSelect')?.addEventListener('change',e=>navigate('energy',e.target.value));bindEnergyInputs();$$('[data-delete-energy]').forEach(b=>b.onclick=()=>deleteEnergyReading(b.dataset.deleteEnergy));
}
...
function renderCosts(){
  setTitle('المصاريف والطوارئ','سجّل الحارس، المولد الخارجي، السولار والصيانة ثم وزّع التكلفة تلقائيًا على السكان.');
  const periods=latestPeriods();if(!state.periodId)state.periodId=periods[0]?.id;const p=selectedPeriod();const rows=p?costsForPeriod(p.id):[];
  const expenseRows=rows.filter(x=>x.direction!=='credit');
  const totalExpense=expenseRows.reduce((a,x)=>a+num(x.amount),0);
  ).innerHTML=`<section class="panel"><div class="panel-head"><div><h2>مصاريف وخدمات الأسبوع</h2><p>أضف حارسًا أو مولدًا خارجيًا أو أي مصروف. يمكنك توزيع قيمته على السكان كما تفعل يدويًا.</p></div><button class="btn primary" id="addCost">+ إضافة مصروف / خدمة</button></div>${!p?empty('افتح أسبوعًا أولًا','المصاريف والخدمات مرتبطة بالأسبوع.'): `<div class="period-picker"><label>الأسبوع الحالي</label><select id="periodSelect" class="period-select">${periods.map(x=>`<option value="${x.id}" ${x.id===p.id?'selected':''}>${safe(x.label||'أسبوع')} — ${fmtDate(x.startDate)} إلى ${fmtDate(x.endDate)}</option>`).join('')}</select></div><div class="money-grid"><div class="money-card"><small>إجمالي المصاريف</small><b>${money(totalExpense)}</b></div><div class="money-card"><small>إجمالي مصاريف التشغيل</small><b>${money(totalExpense)}</b></div></div><div class="section-note" style="margin-top:13px"><b>كيف تستخدمها؟</b> مثال: حارس 1110 ₪ على 37 شخص = 30 ₪ للشخص. أو اكتب مبلغ الشخص الواحد، والبرنامج يحسب الإجمالي. المولد الخارجي يعمل بالطريقة نفسها.</div><div class="table-wrap" style="margin-top:13px"><table class="table"><thead><tr><th>التاريخ</th><th>النوع</th><th>البيان</th><th>المبلغ</th><th>التوزيع</th><th></th></tr></thead><tbody>${expenseRows.length?expenseRows.map(c=>`<tr><td>${fmtDate(c.date)}</td><td>${safe(c.type||'—')}</td><td>${safe(c.description||'—')}</td><td class="strong">${money(c.amount)}</td><td>${c.allocationLabel?safe(c.allocationLabel):'—'}</td><td><button class="mini" data-edit-cost="${c.id}">تعديل</button><button class="mini red" data-delete-cost="${c.id}">حذف</button></td></tr>`).join(''):`<tr><td colspan="6">${empty('لا توجد مصاريف','ابدأ بإضافة خدمة الحارس أو مولد خارجي أو أي مصروف.')}</td></tr>`}</tbody></table></div>`}</section>`;
...
function showCostForm(pid,id){
  ...
  ['cAlloc','cAmount','cPerPerson','cCount'].forEach(id=>$('#'+id).addEventListener('input',updatePreview));updatePreview();$('#cancelCost').onclick=closeModal;
  $('#saveCost').onclick=async()=>{const amount=num($('#cAmount').value);if(amount<=0){toast('اكتب المبلغ الكامل','error');return;}const alloc=$('#cAlloc').value,count=Math.max(0,Math.floor(num($('#cCount').value))),per=num($('#cPerPerson').value);if(alloc!=='none'&&!count){toast('اكتب عدد الأشخاص للتوزيع','error');return;}if(alloc==='per_person'&&per<=0){toast('اكتب مبلغ الشخص الواحد','error');return;}const preview=allocationPreview(alloc,amount,per,count);const data={periodId:pid,type:$('#cType').value,date:$('#cDate').value,amount,direction:'expense',description:$('#cDesc').value.trim()||$('#cType').value,notes:$('#cNotes').value.trim(),allocationRule:alloc,allocationCount:count,perPersonAmount:alloc==='per_person'?per:null,allocatedPerPerson:alloc==='none'?0:preview.each,allocationLabel:alloc==='none'?'بدون توزيع':preview.label,createdBy:c?.createdBy||state.user.uid,updatedAt:serverTimestamp()};
    if(c){await updateDoc(orgDoc('costs',id),data);toast('تم تعديل المصروف.');}
    else{const r=doc(orgCollection('costs'));await setDoc(r,{...data,createdAt:serverTimestamp()}); if(alloc!=='none'&&preview.each>0){const batch=writeBatch(db);for(const sub of subs.slice(0,count)) {const lr=doc(orgCollection('ledger'));batch.set(lr,{subscriberId:sub.id,periodId:pid,transactionType:'SERVICE',serviceCode:data.type==='استئجار مولد خارجي'?'EXTERNAL_GENERATOR':data.type==='خدمة الحارس'?'GUARD':data.type==='تأمين الغاطس'?'PUMP_INSURANCE':'ALLOCATED_COST',debit:preview.each,credit:0,description:data.description,referenceId:r.id,createdAt:serverTimestamp(),createdBy:state.user.uid});}await batch.commit();}}
    state.loaded=false;await loadData(true);closeModal();toast(c?'تم تحديث المصروف':'تم حفظ المصروف وتوزيعه');renderCosts();};
}


function renderContributions(){
  setTitle('المساهمات والخصومات','سجّل أي مساهمة أو خصم لتقليل تكلفة تشغيل المياه. هذه الصفحة منفصلة عن المصاريف.');
  const periods=latestPeriods(); if(!state.periodId) state.periodId=periods[0]?.id; const p=selectedPeriod();
  const rows=p?(state.data.contributions||[]).filter(x=>x.periodId===p.id):[];
  const total=rows.reduce((a,x)=>a+num(x.amount),0);
  $('#app').innerHTML=`<section class="panel">
    <div class="panel-head"><div><h2>المساهمات والخصومات</h2><p>هذه المبالغ تُخصم من تكلفة التشغيل قبل حساب سعر الكوب، ولا تُحمّل مباشرة على السكان.</p></div>
      <div class="panel-actions"><button class="btn primary" id="addContribution">+ إضافة مساهمة / خصم</button></div></div>
    ${!p?empty('افتح أسبوعًا أولًا','المساهمات مرتبطة بالأسبوع.'):`
    <div class="period-picker"><label>الأسبوع الحالي</label><select id="periodSelect" class="period-select">${periods.map(x=>`<option value="${x.id}" ${x.id===p.id?'selected':''}>${safe(x.label||'أسبوع')} — ${fmtDate(x.startDate)} إلى ${fmtDate(x.endDate)}</option>`).join('')}</select></div>
    <div class="money-grid"><div class="money-card"><small>إجمالي الخصومات</small><b>${money(total)}</b></div>
    <div class="money-card"><small>صافي تكلفة التشغيل بعد الخصومات</small><b>${money(Math.max(0,currentTotals(p.id).netCost))}</b></div></div>
    <div class="section-note">مثال: مساهمة 300 ₪ في أسبوع معيّن → تقلل تكلفة التشغيل 300 ₪ قبل حساب سعر الكوب.</div>
    <div class="table-wrap" style="margin-top:13px"><table class="table"><thead><tr><th>التاريخ</th><th>البيان</th><th>المبلغ</th><th>إجراء</th></tr></thead>
    <tbody>${rows.length?rows.map(c=>`<tr><td>${fmtDate(c.date)}</td><td>${safe(c.description||'مساهمة')}</td><td class="strong">${money(c.amount)}</td><td><button class="mini red" data-delete-contribution="${c.id}">حذف</button></td></tr>`).join(''):`<tr><td colspan="4">${empty('لا توجد مساهمات','أضف مساهمة أو خصمًا لهذا الأسبوع.')}</td></tr>`}</tbody></table></div>`}</section>`;
  $('#addContribution').onclick=()=>showContributionForm(p?.id);
  $('#periodSelect')?.addEventListener('change',e=>navigate('contributions',e.target.value));
  $$('[data-delete-contribution]').forEach(b=>b.onclick=()=>deleteContribution(b.dataset.deleteContribution));
}
function showContributionForm(pid,id){
  if(!can('admin','manager','accountant')){toast('إدارة المساهمات مخصصة للإدارة والمحاسبة','error');return;}
  const c=id?(state.data.contributions||[]).find(x=>x.id===id):null;
  openModal(`<h2>${c?'تعديل مساهمة / خصم':'إضافة مساهمة / خصم'}</h2><p class="modal-lead">المبلغ هنا يقلل تكلفة تشغيل المياه قبل حساب سعر الكوب.</p>
  <div class="form-grid"><div class="field"><label>المبلغ</label><input id="crAmount" type="number" min="0" step="0.01" value="${c?.amount??''}"></div>
  <div class="field"><label>التاريخ</label><input id="crDate" type="date" value="${safe(c?.date||dateNow())}"></div>
  <div class="field full"><label>البيان</label><input id="crDesc" value="${safe(c?.description||'مساهمة / خصم')}"></div></div>
  <div class="actions"><button class="btn primary" id="saveContribution">حفظ</button><button class="btn ghost" id="cancelContribution">إلغاء</button></div>`);
  $('#cancelContribution').onclick=closeModal;
  $('#saveContribution').onclick=async()=>{
    const amount=num($('#crAmount').value); if(amount<=0){toast('اكتب مبلغًا صحيحًا','error');return;}
    const data={periodId:pid,amount,date:$('#crDate').value,description:$('#crDesc').value.trim()||'مساهمة / خصم',createdBy:c?.createdBy||state.user.uid,updatedAt:serverTimestamp()};
    if(c) await updateDoc(orgDoc('contributions',id),data); else await setDoc(doc(orgCollection('contributions')),{...data,createdAt:serverTimestamp()});
    closeModal();state.loaded=false;await loadData(true);toast(c?'تم تعديل المساهمة':'تم حفظ المساهمة');renderContributions();
  };
}
async function deleteContribution(id){
  if(!can('admin','manager','accountant')){toast('حذف المساهمة مخصص للإدارة والمحاسبة','error');return;}
  const c=(state.data.contributions||[]).find(x=>x.id===id); if(!c)return;
  if(!confirm(`حذف المساهمة ${money(c.amount)}؟`))return;
  await deleteDoc(orgDoc('contributions',id));
  removeLocal('contributions',id); toast('تم حذف المساهمة'); renderContributions();
}
...
function renderPayments(){
  ...
  $('#app').innerHTML=`<section class="panel"><div class="panel-head"><div><h2>الدفعات</h2><p>الدفع يسجل كحركة ائتمانية، والرصيد يبقى قابلًا للتتبع.</p></div><div class="panel-actions"><button class="btn soft" id="exportPayments">↓ Excel</button><button class="btn primary" id="newPay">+ تسجيل دفعة</button></div></div><div class="toolbar"><input id="paySearch" class="search" placeholder="ابحث باسم الساكن أو رقم الإيصال…"></div><div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>الساكن</th><th>المبلغ</th><th>الطريقة</th><th>الإيصال</th><th>الرصيد الحالي</th><th></th></tr></thead><tbody id="payBody">${paymentRows(pays)}</tbody></table></div></section>`;$('#newPay').onclick=showPaymentForm;$('#exportPayments').onclick=()=>exportPayments(pays);$('#paySearch').oninput=e=>$('#payBody').innerHTML=paymentRows(pays.filter(p=>{const s=(state.data.subscribers||[]).find(x=>x.id===p.subscriberId);return [s?.name,p.receiptNumber].some(v=>String(v||'').includes(e.target.value))}));
  $$('[data-delete-payment]').forEach(b=>b.onclick=()=>deletePayment(b.dataset.deletePayment));
}
...
function showPaymentForm(){const subs=(state.data.subscribers||[]).filter(s=>s.active!==false);openModal(`<h2>تسجيل دفعة</h2><p class="modal-lead">يمكن أن تكون الدفعة لسداد رصيد أو دفعة تحت الحساب.</p><div class="form-grid"><div class="field full"><label>الساكن</label><select id="paySub"><option value="">اختر الساكن</option>${subs.map(s=>`<option value="${s.id}">${safe(s.code)} — ${safe(s.name)}</option>`).join('')}</select></div><div class="field"><label>المبلغ</label><input id="payAmount" type="number" min="0" step="0.01"></div><div class="field"><label>التاريخ</label><input id="payDate" type="date" value="${dateNow()}"></div><div class="field"><label>طريقة الدفع</label><select id="payMethod"><option>نقدي</option><option>تحويل بنكي</option><option>أخرى</option></select></div><div class="field"><label>رقم الإيصال</label><input id="payReceipt" placeholder="اختياري"></div><div class="field full"><label>ملاحظة</label><textarea id="payNote"></textarea></div></div><div class="actions"><button class="btn primary" id="savePay">حفظ الدفعة</button><button class="btn ghost" id="cancelPay">إلغاء</button></div>`);$('#cancelPay').onclick=closeModal;$('#savePay').onclick=async()=>{const subscriberId=$('#paySub').value,amount=num($('#payAmount').value);if(!subscriberId||amount<=0){toast('اختر الساكن واكتب مبلغًا صحيحًا','error');return;}const ref=doc(orgCollection('payments'));const lr=doc(orgCollection('ledger'));const payment={subscriberId,periodId:selectedPeriod()?.id||null,amount,paymentDate:$('#payDate').value,method:$('#payMethod').value,receiptNumber:$('#payReceipt').value.trim(),note:$('#payNote').value.trim(),createdBy:state.user.uid,createdAt:serverTimestamp()};const batch=writeBatch(db);batch.set(ref,payment);batch.set(lr,{subscriberId,periodId:payment.periodId,transactionType:'PAYMENT',credit:amount,debit:0,description:'دفعة',referenceId:ref.id,paymentDate:payment.paymentDate,createdAt:serverTimestamp(),createdBy:state.user.uid});await batch.commit();closeModal();state.loaded=false;await loadData(true);toast('تم تسجيل الدفعة');renderPayments();};}



async function deleteCost(id){
  if(!can('admin','manager','accountant')){toast('حذف المصروف مخصص للإدارة والمحاسبة','error');return;}
  const c=(state.data.costs||[]).find(x=>x.id===id);if(!c)return;
  if(!confirm(`حذف "${c.description||c.type}" بقيمة ${money(c.amount)}؟\nسيتم حذف الحركات التي أنشأها هذا المصروف أيضًا.`))return;
  const related=(state.data.ledger||[]).filter(x=>x.referenceId===id);
  const batch=writeBatch(db);batch.delete(orgDoc('costs',id));for(const r of related)batch.delete(orgDoc('ledger',r.id));
  await batch.commit();state.loaded=false;await loadData(true);toast('تم حذف المصروف');renderCosts();
}
async function deleteDebt(id){
  if(!can('admin','manager','accountant')){toast('حذف الدين مخصص للإدارة والمحاسبة','error');return;}
  const d=(state.data.debts||[]).find(x=>x.id===id);if(!d)return;
  if(!confirm(`حذف الدين "${d.description||'دين سابق'}" بقيمة ${money(d.amount)}؟`))return;
  const batch=writeBatch(db);batch.delete(orgDoc('debts',id));
  for(const r of (state.data.ledger||[]).filter(x=>x.referenceId===id))batch.delete(orgDoc('ledger',r.id));
  await batch.commit();state.loaded=false;await loadData(true);toast('تم حذف الدين');renderDebts();
}
async function deletePayment(id){
  if(!can('admin','manager','accountant')){toast('حذف الدفعة مخصص للإدارة والمحاسبة','error');return;}
  const pay=(state.data.payments||[]).find(x=>x.id===id);if(!pay)return;
  if(!confirm(`حذف دفعة ${money(pay.amount)} للساكن؟\nسيعود أثرها إلى الرصيد.`))return;
  const batch=writeBatch(db);batch.delete(orgDoc('payments',id));for(const r of (state.data.ledger||[]).filter(x=>x.referenceId===id))batch.delete(orgDoc('ledger',r.id));
  await batch.commit();state.loaded=false;await loadData(true);toast('تم حذف الدفعة');renderPayments();
}
...
function renderDebts(){
  setTitle('الديون السابقة','سجّل أي مبلغ متبقٍ على الساكن حتى يبقى ظاهرًا في كشفه ورسالة واتساب.');
  ...
  const debts=[...(state.data.debts||[])].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
  ...
  $('#app').innerHTML=`<section class="panel"><div class="panel-head"><div><h2>الديون السابقة</h2><p>أضف الدين كحركة مستقلة. لا يتم إخفاؤه عند تسجيل دفعة جزئية.</p></div><button class="btn primary" id="addDebt">+ إضافة دين</button></div><div class="money-grid"><div class="money-card"><small>إجمالي الديون</small><b>${money(debts.reduce((a,d)=>a+num(d.amount),0))}</b></div><div class="money-card"><small>المسدد</small><b>${money(debts.reduce((a,d)=>a+num(d.paidAmount),0))}</b></div><div class="money-card"><small>المتبقي</small><b>${money(debts.reduce((a,d)=>a+Math.max(0,num(d.amount)-num(d.paidAmount)),0))}</b></div></div><div class="table-wrap" style="margin-top:13px"><table class="table"><thead><tr><th>التاريخ</th><th>الساكن</th><th>البيان</th><th>الدين</th><th>المسدد</th><th>المتبقي</th><th>إجراء</th></tr></thead><tbody>${debts.length?debts.map(d=>{const s=subs.find(x=>x.id===d.subscriberId);return `<tr><td>${fmtDate(d.date)}</td><td>${safe(s?.name||'—')}</td><td>${safe(d.description||'—')}</td><td>${money(d.amount)}</td><td>${money(d.paidAmount||0)}</td><td class="strong">${money(Math.max(0,num(d.amount)-num(d.paidAmount)))}</td><td><button class="mini red" data-delete-debt="${d.id}">حذف</button></td></tr>`}).join(''):`<tr><td colspan="6">${empty('لا توجد ديون مسجلة','عند بدء الحساب الفعلي يمكنك تسجيل الرصيد القديم هنا.')}</td></tr>`}</tbody></table></div></section>`;
  $('#addDebt').onclick=showDebtForm;$$('[data-delete-debt]').forEach(b=>b.onclick=()=>deleteDebt(b.dataset.deleteDebt));
}
function showDebtForm(){
  const subs=(state.data.subscribers||[]).filter(s=>s.active!==false).sort((a,b)=>String(a.code).localeCompare(String(b.code),undefined,{numeric:true}));
  openModal(`<h2>إضافة دين</h2><p class="modal-lead">مثال: متأخرات مياه قديمة، صيانة قديمة، أو مبلغ لم يُسدّد.</p><div class="form-grid"><div class="field full"><label>الساكن</label><select id="dSub">${subs.map(s=>`<option value="${s.id}">${safe(s.code)} — ${safe(s.name)}</option>`).join('')}</select></div><div class="field"><label>المبلغ</label><input id="dAmount" type="number" min="0" step="0.01"></div><div class="field"><label>التاريخ</label><input id="dDate" type="date" value="${dateNow()}"></div><div class="field full"><label>البيان</label><input id="dDesc" placeholder="مثال: دين سابق للمياه"></div></div><div class="actions"><button class="btn primary" id="saveDebt">حفظ الدين</button><button class="btn ghost" id="cancelDebt">إلغاء</button></div>`);
  $('#cancelDebt').onclick=closeModal;$('#saveDebt').onclick=async()=>{const subscriberId=$('#dSub').value,amount=num($('#dAmount').value);if(!subscriberId||amount<=0){toast('اختر الساكن واكتب مبلغ الدين','error');return;}const ref=doc(orgCollection('debts'));const lr=doc(orgCollection('ledger'));const data={subscriberId,amount,paidAmount:0,date:$('#dDate').value,description:$('#dDesc').value.trim()||'دين سابق',createdAt:serverTimestamp(),createdBy:state.user.uid};const batch=writeBatch(db);batch.set(ref,data);batch.set(lr,{subscriberId,transactionType:'DEBT',debit:amount,credit:0,description:data.description,referenceId:ref.id,date:data.date,createdAt:serverTimestamp(),createdBy:state.user.uid});await batch.commit();state.loaded=false;await loadData(true);closeModal();toast('تم تسجيل الدين');renderDebts();};
}
...
function subscriberFinancialSummary(id,periodId=null){
  ...
  const currentConsumption=(state.data.ledger||[]).filter(x=>x.subscriberId===id&&x.periodId===p?.id&&x.transactionType==='WATER').reduce((a,x)=>{const rr=(state.data.readings||[]).find(r=>r.id===x.referenceId);return a+(rr?.consumption!=null?num(rr.consumption):0)},0);
  const services=current.filter(x=>x.transactionType==='SERVICE').reduce((a,x)=>a+num(x.debit),0);
  const debtsCurrent=current.filter(x=>x.transactionType==='DEBT').reduce((a,x)=>a+num(x.debit),0);
  const other=current.filter(x=>!['WATER','SERVICE','PAYMENT','DEBT'].includes(x.transactionType)).reduce((a,x)=>a+num(x.debit),0);
  ...
  const unpaidDebts=(state.data.debts||[]).filter(d=>d.subscriberId===id).reduce((a,d)=>a+Math.max(0,num(d.amount)-num(d.paidAmount)),0);
  const finalBalance=previousBalance+currentWater+services+debtsCurrent+other-periodPayments;
  return {s,p,previousBalance,currentWater,currentConsumption,services,debtsCurrent,other,periodPayments,totalPayments,unpaidDebts,finalBalance};
}
function messageForResident(sum){
  ...
  const prior=Math.max(0,sum.previousBalance);
  const currentCharges=Math.max(0,sum.currentWater+sum.services+sum.debtsCurrent+sum.other);
  const paymentsNow=Math.max(0,sum.periodPayments);
  const total=Math.max(0,prior+currentCharges-paymentsNow);
  return `السلام عليكم ${name}\nتفاصيل حساب عمارة الأمين حتى يوم ${day}\nسحب المياه هذا الأسبوع : ${fmt(sum.currentConsumption||0,3)} كوب\nقيمة مياه الأسبوع : ${money(sum.currentWater)}\nخدمات الحارس + تأمين الغاطس : ${money(sum.services)}\nالرصيد السابق + الديون السابقة : ${money(prior)}\nالدفعات المسجلة : ${money(paymentsNow)}\nالإجمالي المطلوب : ${money(total)}\nوشكرا لتعاونكم`;
}
...
async function deleteWeek(pid){if(!pid){toast('اختر أسبوعًا','error');return;}if(!can('admin')){toast('حذف الأسبوع مخصص للمدير','error');return;}const p=periodById(pid);if(!confirm(`سيتم حذف ${p?.label||'الأسبوع'} مع قراءات الماء والكهرباء والمصاريف وحركات المياه المرتبطة به.\n\nهل أنت متأكد؟`))return;const batch=writeBatch(db);batch.delete(orgDoc('periods',pid));for(const c of ['readings','energyReadings','costs','waterSummary'])for(const r of (state.data[c]||[]).filter(x=>x.periodId===pid))batch.delete(orgDoc(c,r.id));for(const t of (state.data.ledger||[]).filter(x=>x.periodId===pid))batch.delete(orgDoc('ledger',t.id));const seedPeriod=(p?.seedKey?INITIAL_DATA.periods.find(x=>`periods/${x.id}`===p.seedKey):null)||INITIAL_DATA.periods.find(x=>x.id===pid);if(seedPeriod)batch.set(orgDoc('seedDeletes',`periods__${seedPeriod.id}`),{key:seedDeleteKey('periods',seedPeriod.id),deletedAt:serverTimestamp(),deletedBy:state.user.uid});await batch.commit();state.loaded=false;await loadData(true);state.periodId=null;toast('تم حذف الأسبوع');renderSettings();}
