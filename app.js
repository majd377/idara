...
async function loadData(force=false){
  ...
  state.loading=true;
  try{const results=await Promise.all(COLLECTIONS.map(async c=>{try{const snap=await getDocs(orgCollection(c));return [c,snap.docs.map(d=>({id:d.id,...d.data()}))]}catch(e){console.warn('Load skipped:',c,e);return [c,[]];}}));for(const [c,rows] of results)state.data[c]=rows;state.loaded=true;}
...
function currentTotals(pid){
  ...
  const expenseTotal=cs.filter(c=>c.direction!=='credit').reduce((a,c)=>a+num(c.amount),0);const contributionTotal=(state.data.contributions||[]).filter(c=>c.periodId===pid).reduce((a,c)=>a+num(c.amount),0);const extras=expenseTotal-contributionTotal;
...
function readingTable(pid){
  ...
  const p=periodById(pid);return `<div class="table-wrap"><table class="table" style="min-width:980px"><thead><tr><th>الكود</th><th>الساكن</th><th>البناية / النوع</th><th>القراءة السابقة</th><th>القراءة الحالية</th><th>السحب</th><th>سعر الكوب</th><th>القيمة</th><th>الحالة</th></tr></thead><tbody>${rows.length?rows.map(x=>{const {s,r}=x;const cons=r.currentReading!=null&&r.previousReading!=null?Math.max(0,num(r.currentReading)-num(r.previousReading))/1000:null;const price=r.unitPrice??p.waterUnitPrice??'';const charge=cons!=null&&price!==''?cons*num(price):null;return `<tr data-rid="${r.id}"><td><span class="code">${safe(s.code)}</span></td><td><b>${safe(s.name)}</b></td><td>${s.type==='خارجي'?'<span class="badge warn">خارجي</span>':safe(subscriberRow(s).buildingName)}</td><td><input class="reading-input prev" type="number" step="0.001" value="${r.previousReading??''}"></td><td><input class="reading-input current" type="number" step="0.001" value="${r.currentReading??''}"></td><td class="cons">${cons==null?'—':fmt(cons,3)}</td><td><input class="reading-input price" type="number" step="0.01" value="${price}"></td><td class="charge">${charge==null?'—':money(charge)}</td><td class="status">${r.currentReading==null?'<span class="badge warn">بانتظار</span>':((num(r.currentReading)<num(r.previousReading))?'<span class="badge danger">تحقق</span>':'<span class="badge ok">جاهزة</span>')}</td><td><button class="mini red" data-delete-reading="${r.id}">حذف</button></td></tr>`}).join(''):`<tr><td colspan="9">${empty('لا توجد قراءات','تأكد من وجود سكان وعدادات قبل فتح الأسبوع.')}</td></tr>`}</tbody></table></div><div class="readings-actions"><span class="muted">حفظ القراءة لا يعتمد على حساب السعر؛ الحساب النهائي يتم من صفحة «الأسابيع والحساب».</span><button class="btn primary" id="saveReadings">حفظ كل القراءات</button></div>`;
}
...
async function deleteReading(id){if(!can('admin','manager','accountant','operator')){toast('حذف القراءة غير مسموح','error');return;}if(!confirm('حذف قراءة المياه هذه؟'))return;await deleteDoc(orgDoc('readings',id));removeLocal('readings',id);toast('تم حذف القراءة');renderReadings();}
async function deleteEnergyReading(id){if(!can('admin','manager','accountant')){toast('حذف قراءة الكهرباء مخصص للإدارة','error');return;}if(!confirm('حذف قراءة الكهرباء هذه؟'))return;await deleteDoc(orgDoc('energyReadings',id));removeLocal('energyReadings',id);toast('تم حذف قراءة الكهرباء');renderEnergy();}
async function deleteContribution(id){if(!can('admin','manager','accountant')){toast('حذف المساهمة غير مسموح','error');return;}if(!confirm('حذف هذه المساهمة/الخصم؟'))return;await deleteDoc(orgDoc('contributions',id));removeLocal('contributions',id);toast('تم حذف المساهمة');renderContributions();}
function renderContributions(){setTitle('المساهمات والخصومات','تُخصم من تكلفة التشغيل قبل حساب سعر الكوب.');const ps=latestPeriods();if(!state.periodId)state.periodId=ps[0]?.id;const p=selectedPeriod();const rows=p?(state.data.contributions||[]).filter(x=>x.periodId===p.id):[];$('#app').innerHTML=`<section class=\"panel\"><div class=\"panel-head\"><div><h2>المساهمات والخصومات</h2><p>هذه المبالغ تقلل تكلفة التشغيل ولا تُحمّل على السكان.</p></div><button class=\"btn primary\" id=\"addContribution\">+ إضافة مساهمة / خصم</button></div>${p?`<div class=\"period-picker\"><label>الأسبوع</label><select id=\"periodSelect\" class=\"period-select\">${ps.map(x=>`<option value=\"${x.id}\" ${x.id===p.id?'selected':''}>${safe(x.label||'أسبوع')} — ${fmtDate(x.startDate)} إلى ${fmtDate(x.endDate)}</option>`).join('')}</select></div><div class=\"money-grid\"><div class=\"money-card\"><small>إجمالي الخصومات</small><b>${money(rows.reduce((t,x)=>t+num(x.amount),0))}</b></div></div><div class=\"table-wrap\"><table class=\"table\"><thead><tr><th>التاريخ</th><th>البيان</th><th>المبلغ</th><th>إجراء</th></tr></thead><tbody>${rows.length?rows.map(x=>`<tr><td>${fmtDate(x.date)}</td><td>${safe(x.description||'خصم')}</td><td>${money(x.amount)}</td><td><button class=\"mini red\" data-del-contrib=\"${x.id}\">حذف</button></td></tr>`).join(''):`<tr><td colspan=\"4\">${empty('لا توجد مساهمات','أضف مساهمة أو خصمًا من الزر أعلاه.')}</td></tr>`}</tbody></table></div>`:empty('لا يوجد أسبوع','أنشئ أسبوعًا أولًا.')}</section>`;$('#addContribution').onclick=()=>showContributionForm(p?.id);$('#periodSelect')?.addEventListener('change',e=>navigate('contributions',e.target.value));$$('[data-del-contrib]').forEach(b=>b.onclick=()=>deleteContribution(b.dataset.delContrib));}
function showContributionForm(pid){if(!can('admin','manager','accountant')){toast('إدارة المساهمات مخصصة للإدارة والمحاسبة','error');return;}openModal(`<h2>إضافة مساهمة / خصم</h2><div class=\"form-grid\"><div class=\"field\"><label>المبلغ</label><input id=\"crAmount\" type=\"number\" step=\"0.01\"></div><div class=\"field\"><label>التاريخ</label><input id=\"crDate\" type=\"date\" value=\"${dateNow()}\"></div><div class=\"field full\"><label>البيان</label><input id=\"crDesc\" value=\"مساهمة / خصم\"></div></div><div class=\"actions\"><button class=\"btn primary\" id=\"saveContribution\">حفظ</button><button class=\"btn ghost\" id=\"cancelContribution\">إلغاء</button></div>`);$('#cancelContribution').onclick=closeModal;$('#saveContribution').onclick=async()=>{const amount=num($('#crAmount').value);if(amount<=0){toast('اكتب مبلغًا صحيحًا','error');return;}await setDoc(doc(orgCollection('contributions')),{periodId:pid,amount,date:$('#crDate').value,description:$('#crDesc').value.trim()||'مساهمة / خصم',createdBy:state.user.uid,createdAt:serverTimestamp()});closeModal();state.loaded=false;await loadData(true);toast('تم حفظ المساهمة');renderContributions();};}
async function deleteCost(id){if(!can('admin','manager','accountant')){toast('حذف المصروف غير مسموح','error');return;}if(!confirm('حذف هذا المصروف؟'))return;const rel=(state.data.ledger||[]).filter(x=>x.referenceId===id);const b=writeBatch(db);b.delete(orgDoc('costs',id));rel.forEach(r=>b.delete(orgDoc('ledger',r.id)));await b.commit();state.loaded=false;await loadData(true);toast('تم حذف المصروف');renderCosts();}
async function deleteDebt(id){if(!can('admin','manager','accountant')){toast('حذف الدين غير مسموح','error');return;}if(!confirm('حذف هذا الدين؟'))return;const b=writeBatch(db);b.delete(orgDoc('debts',id));(state.data.ledger||[]).filter(x=>x.referenceId===id).forEach(r=>b.delete(orgDoc('ledger',r.id)));await b.commit();state.loaded=false;await loadData(true);toast('تم حذف الدين');renderDebts();}
async function deletePayment(id){if(!can('admin','manager','accountant')){toast('حذف الدفعة غير مسموح','error');return;}if(!confirm('حذف هذه الدفعة؟'))return;const b=writeBatch(db);b.delete(orgDoc('payments',id));(state.data.ledger||[]).filter(x=>x.referenceId===id).forEach(r=>b.delete(orgDoc('ledger',r.id)));await b.commit();state.loaded=false;await loadData(true);toast('تم حذف الدفعة');renderPayments();}
...
function subscriberFinancialSummary(id,periodId=null){
  ...
  return {s,p,previousBalance,currentWater,services,debtsCurrent,other,periodPayments,totalPayments,unpaidDebts,finalBalance};
}
function messageForResident(sum){
  const name=sum.s.name||'الساكن'; const p=sum.p; const end=p?.endDate||dateNow(); const dayName=new Date(end+'T12:00:00').toLocaleDateString('ar-PS',{weekday:'long'}); const day=`${dayName} ${fmtDate(end)}`;
  const prior=Math.max(0,sum.previousBalance); const currentCharges=Math.max(0,sum.currentWater+sum.services+sum.debtsCurrent+sum.other); const paymentsNow=Math.max(0,sum.periodPayments);
  const currentConsumption=(state.data.readings||[]).filter(r=>r.periodId===p?.id && (r.subscriberId===sum.s.id || r.subscriberCode===sum.s.code)).reduce((a,r)=>a+num(r.consumption),0);
  const total=Math.max(0,prior+currentCharges-paymentsNow);
  return `السلام عليكم ${name}\nتفاصيل حساب عمارة الأمين حتى يوم ${day}\nسحب المياه هذا الأسبوع : ${fmt(currentConsumption,3)} كوب\nقيمة مياه الأسبوع : ${money(sum.currentWater)}\nخدمات الحارس + تأمين الغاطس : ${money(sum.services)}\nالرصيد السابق + الديون السابقة : ${money(prior)}\nالدفعات المسجلة : ${money(paymentsNow)}\nالإجمالي قبل الدفعات : ${money(prior+currentCharges)}\nالإجمالي المطلوب : ${money(total)}\nوشكرا لتعاونكم`;
}
