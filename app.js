...
function renderSubscribers(){
  ...
  $('#app').innerHTML=`<section class="grid-2">
    <div class="panel">
      ...
      <div class="table-wrap"><table class="table" style="min-width:900px"><thead><tr><th>الكود</th><th>الاسم</th><th>النوع</th><th>البناية</th><th>الوحدة</th><th>المديونية</th><th>إجراءات</th></tr></thead><tbody id="subBody">${subscriberRowsSorted(rows,'code-asc')}</tbody></table></div>
    </div>
    <div class="panel">
...
function showPaymentForm(){
  ...
  openModal(`<h2>تسجيل دفعة</h2><p class="modal-lead">الدفعة تسجل مرة واحدة فقط. في الأسبوع التالي ينتقل أثرها على المديونية/الرصيد الدائن، ولا تتكرر كدفعة جديدة.</p>
  <div class="form-grid">
  <div class="field full"><label>الساكن</label><select id="paySub"><option value="">اختر الساكن</option>${subs.map(s=>`<option value="${s.id}">${safe(s.code)} — ${safe(s.name)}</option>`).join('')}</select></div>
  <div class="field"><label>الأسبوع</label><select id="payPeriod"><option value="">دفعة عامة / تحت الحساب</option>${periods.map(p=>`<option value="${p.id}" ${p.id===state.periodId?'selected':''}>${safe(p.label)} — ${fmtDate(p.startDate)}</option>`).join('')}</select></div>
  <div class="field"><label>المبلغ</label><input id="payAmount" type="number" min="0" step="0.01"></div>
  <div class="field"><label>التاريخ</label><input id="payDate" type="date" value="${dateNow()}"></div>
  <div class="field"><label>طريقة الدفع</label><select id="payMethod"><option>نقدي</option><option>تحويل بنكي</option><option>أخرى</option></select></div>
  <div class="field"><label>رقم الإيصال</label><input id="payReceipt" placeholder="اختياري"></div>
  <div class="field full"><label>ملاحظة</label><textarea id="payNote"></textarea></div>
  </div><div class="actions"><button class="btn primary" id="savePay">حفظ الدفعة</button><button class="btn ghost" id="cancelPay">إلغاء</button></div>`);
  ...
  $('#savePay').onclick=async()=>{
    ...
    const ref=doc(orgCollection('payments')),lr=doc(orgCollection('ledger'));
    const payment={subscriberId,periodId,amount,paymentDate,method:$('#payMethod').value,receiptNumber:$('#payReceipt').value.trim(),note:$('#payNote').value.trim(),createdBy:state.user.uid,createdAt:serverTimestamp()};
    const batch=writeBatch(db);batch.set(ref,payment);batch.set(lr,{subscriberId,periodId,transactionType:'PAYMENT',credit:amount,debit:0,description:'دفعة',referenceId:ref.id,paymentDate,createdAt:serverTimestamp(),createdBy:state.user.uid});
    await batch.commit();closeModal();state.loaded=false;await loadData(true);toast('تم تسجيل الدفعة');renderPayments();
  };
}
...
function renderReports(){
  ...
  const periods=latestPeriods(),subs=(state.data.subscribers||[]).filter(s=>s.active!==false).sort((a,b)=>String(a.code).localeCompare(String(b.code),undefined,{numeric:true}));
  const defaultWeek=state.periodId||periods[0]?.id||'';
  ...
  <section class="report-card report-card-wide">
    ...
    <div class="form-grid">
      ...
      <div class="field"><label>من أسبوع للجدول/PDF</label><select id="repFrom"><option value="">الأقدم</option>${periods.map(p=>`<option value="${p.id}">${safe(p.label)} — ${fmtDate(p.startDate)}</option>`).join('')}</select></div>
      <div class="field"><label>إلى أسبوع للجدول/PDF</label><select id="repTo"><option value="">الأحدث</option>${periods.map(p=>`<option value="${p.id}" ${p.id===defaultWeek?'selected':''}>${safe(p.label)} — ${fmtDate(p.startDate)}</option>`).join('')}</select></div>
    </div>
    <div class="actions"><button class="btn primary" id="showResidentReport">عرض الكشف</button><button class="btn soft" id="downloadResidentPdf" disabled>↓ تنزيل كشف PDF</button></div>
  </section>
  ...
  $('#showResidentReport').onclick=()=>{
    ...
    const fromId=$('#repFrom').value||periods[0]?.id,toId=$('#repTo').value||periods[periods.length-1]?.id;
    renderResidentReportCard(sid,wid,fromId,toId);$('#downloadResidentPdf').disabled=false;
  };
...
function weekPreviousBalance(id,p){
  return (state.data.ledger||[]).filter(x=>x.subscriberId===id&&transactionDate(x)&&transactionDate(x)<String(p.startDate)).reduce((a,x)=>a+num(x.debit)-num(x.credit),0);
}
function paymentsForWeek(id,p){
  return (state.data.ledger||[]).filter(x=>x.subscriberId===id&&x.transactionType==='PAYMENT'&&(
    x.periodId===p.id||(!x.periodId&&transactionDate(x)>=String(p.startDate)&&transactionDate(x)<=String(p.endDate))
  )).reduce((a,x)=>a+num(x.credit),0);
}
function subscriberFinancialSummary(id,periodId=null){
  ...
  if(!p){const b=balanceOf(id);return {s,p:null,previousBalance:b,previousDebt:Math.max(0,b),previousCredit:Math.max(0,-b),currentConsumption:0,currentWater:0,guard:0,other:0,periodPayments:0,totalPaymentsAll:0,finalBeforePayments:Math.max(0,b),finalBalance:Math.max(0,b),creditCarry:Math.max(0,-b),appliedPrice:0};}
  const previousBalance=weekPreviousBalance(id,p),previousDebt=Math.max(0,previousBalance),previousCredit=Math.max(0,-previousBalance);
  ...
  const currentWater=currentConsumption*num(price);
  const currentLedger=(state.data.ledger||[]).filter(x=>x.subscriberId===id&&(x.periodId===p.id||(x.transactionType==='PAYMENT'&&!x.periodId&&transactionDate(x)>=String(p.startDate)&&transactionDate(x)<=String(p.endDate))));
  const guard=currentLedger.filter(x=>x.transactionType==='SERVICE'&&x.serviceCode==='GUARD').reduce((a,x)=>a+num(x.debit)-num(x.credit),0);
  const other=currentLedger.filter(x=>{if(['PAYMENT','WATER','DEBT'].includes(x.transactionType))return false;if(x.transactionType==='SERVICE'&&x.serviceCode==='GUARD')return false;return ['SERVICE','ALLOCATED_COST','ADJUSTMENT','OTHER'].includes(x.transactionType);}).reduce((a,x)=>a+num(x.debit)-num(x.credit),0);
  const periodPayments=paymentsForWeek(id,p);
  const totalPaymentsAll=(state.data.ledger||[]).filter(x=>x.subscriberId===id&&x.transactionType==='PAYMENT').reduce((a,x)=>a+num(x.credit),0);
  const beforePayments=Math.max(0,previousDebt+currentWater+guard+other-previousCredit);
  const finalBalance=Math.max(0,beforePayments-periodPayments),creditCarry=Math.max(0,periodPayments-beforePayments);
  return {s,p,previousBalance,previousDebt,previousCredit,currentConsumption,currentWater,guard,services:guard,other,periodPayments,totalPaymentsAll,finalBeforePayments:beforePayments,finalBalance,creditCarry,appliedPrice:price};
}
...
async function downloadResidentPdf(id,weekId,fromId,toId){
  ...
  const tableSection=source.querySelector('.account-section');if(!tableSection){toast('تعذر العثور على جدول الكشف','error');return;}
  const clone=tableSection.cloneNode(true),host=document.createElement('div');host.dir='rtl';host.style.cssText='position:absolute;left:-100000px;top:0;width:1400px;background:#fff;padding:24px;direction:rtl;font-family:Cairo,Arial,sans-serif;color:#183734;';
  const title=document.createElement('div');title.innerHTML=`<h2 style="margin:0 0 5px">كشف حساب — ${safe(source.querySelector('h2')?.textContent||'الساكن')}</h2><p style="margin:0 0 14px;color:#66736f">من ${safe(fmtDate(periodById(fromId)?.startDate)||'الأقدم')} إلى ${safe(fmtDate(periodById(toId)?.endDate)||'الأحدث')}</p>`;
  host.appendChild(title);host.appendChild(clone);document.body.appendChild(host);
  const filename=`كشف_حساب_${String(source.querySelector('h2')?.textContent||'ساكن').replace(/[\\/:*?"<>|]+/g,'_')}.pdf`;
  try{await html2pdf().set({margin:8,filename,image:{type:'jpeg',quality:.95},html2canvas:{scale:1.7,useCORS:true,backgroundColor:'#ffffff'},jsPDF:{unit:'mm',format:'a4',orientation:'landscape'},pagebreak:{mode:['avoid-all','css','legacy']}}).from(host).save();toast('تم تنزيل كشف الحساب PDF');}catch(e){console.error(e);toast('تعذر إنشاء PDF','error');}finally{host.remove();}
}
...
function exportSubscribers(rows){exportXlsx(rows.map(s=>({الكود:s.code,الاسم:s.name,النوع:s.type,البناية:s.buildingName,الوحدة:s.unitCode,الهاتف:s.phone||'',المديونية:s.debt||0})),'السكان','سكان_عمارة_الأمين.xlsx');}
function exportBalances(rows){exportXlsx(rows.map(s=>({الكود:s.code,الاسم:s.name,البناية:s.buildingName,الوحدة:s.unitCode,المديونية:s.debt||0})),'الأرصدة','أرصدة_عمارة_الأمين.xlsx');}
