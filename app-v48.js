...
const waterChargeAmount=(consumption,price)=>Number(consumption||0)*Number(price||0);
const waterPriceText=v=>{const n=Number(v);return Number.isFinite(n)?new Intl.NumberFormat('en-US',{minimumFractionDigits:1,maximumFractionDigits:1,useGrouping:true}).format(n):'—';};
...
const fmt=(v,d=1)=>new Intl.NumberFormat('en-US',{maximumFractionDigits:d}).format(Number(v||0));
...
function renderSubscribers(){
  ...
  $('#app').innerHTML=`<section class="grid-2 subscribers-page-grid">
    <div class="panel">
...
function renderResidentReportCard(id,weekId,fromId,toId){
  ...
  const rows=periods.map(p=>{const x=subscriberFinancialSummary(id,p.id);return `<tr>
    <td><b>${safe(p.label||'أسبوع')}</b><br><small>${fmtDate(p.startDate)} → ${fmtDate(p.endDate)}</small></td>
    <td>${fmt(x.currentConsumption,3)} كوب</td><td>${money(x.currentWater)}</td>
...
function residentPrintHtml(source,titleText='كشف حساب'){
  ...
  *{box-sizing:border-box}
  html,body{margin:0;padding:0;background:#fff;color:#183734;font-family:Arial,"Cairo",sans-serif;direction:rtl;width:100%;max-width:100%;}
  body{min-height:194mm}
  .print-sheet{width:281mm;max-width:281mm;min-height:194mm;margin:0 auto;padding:0;overflow:hidden;background:#fff}
  .resident-report-head{margin:0 0 8px;display:flex;justify-content:space-between;gap:12px;align-items:flex-start}
  .resident-report-head h2{margin:0 0 4px;font-size:18px}.resident-report-head p{margin:0;color:#5f6c67;font-size:10px}
  .balance-box{padding:8px;background:#eef6f3;border-radius:8px;display:inline-block;margin-top:6px}.balance-box span{display:block;font-size:7px;color:#6c7a76}.balance-box b{font-size:15px}
  .resident-report-card{width:100%;max-width:100%}.resident-ledger-table{width:100%;max-width:100%;overflow:visible!important}
  .table{width:100%!important;max-width:100%!important;min-width:0!important;border-collapse:collapse;table-layout:fixed;font-size:6.8px;direction:rtl}
  .table th,.table td{border:1px solid #cdd9d6;padding:3.5px 3px;text-align:center;vertical-align:middle;white-space:normal!important;word-break:break-word;overflow-wrap:anywhere;line-height:1.25}
  .table th{background:#edf5f2;font-weight:800}.table td:first-child{text-align:right}
  .table th:nth-child(1),.table td:nth-child(1){width:11%}.table th:nth-child(2),.table td:nth-child(2){width:9%}.table th:nth-child(3),.table td:nth-child(3){width:11%}.table th:nth-child(4),.table td:nth-child(4){width:10%}.table th:nth-child(5),.table td:nth-child(5){width:11%}.table th:nth-child(6),.table td:nth-child(6){width:11%}.table th:nth-child(7),.table td:nth-child(7){width:13%}.table th:nth-child(8),.table td:nth-child(8){width:10%}.table th:nth-child(9),.table td:nth-child(9){width:14%}
  @page{size:A4 landscape;margin:8mm}
  @media print{button,.actions,.message-head{display:none!important}body{width:281mm}.print-sheet{width:281mm;max-width:281mm;min-height:194mm}}
  </style></head><body><div class="print-sheet"><div>${title?title.outerHTML:''}</div>${tableClone.outerHTML}</div></body></html>`;
}
...
async function downloadReportElementPdf(source,s,periods){
  ...
  iframe.style.cssText='position:fixed;width:1100px;height:900px;left:-12000px;top:-12000px;border:0;opacity:0;visibility:hidden';
  document.body.appendChild(iframe);
  try{
    const idoc=iframe.contentDocument;idoc.open();idoc.write(html);idoc.close();
    await new Promise(resolve=>setTimeout(resolve,350));
    try{await idoc.fonts?.ready;}catch{}
    const page=idoc.querySelector('.print-sheet');
    if(!page)throw new Error('صفحة الطباعة غير موجودة');
    const filename=`كشف_حساب_${String(s?.name||'ساكن').replace(/[\/:*?"<>|]+/g,'_')}.pdf`;
    await window.html2pdf().set({
      margin:[8,8,8,8],
      filename,
      image:{type:'jpeg',quality:.98},
      html2canvas:{scale:2,useCORS:true,backgroundColor:'#ffffff',logging:false,scrollX:0,scrollY:0,windowWidth:1065,windowHeight:740,width:1065},
      jsPDF:{unit:'mm',format:'a4',orientation:'landscape',compress:true},
      pagebreak:{mode:['css','legacy'],avoid:['tr','.table tr','thead']}
    }).from(page).save();
