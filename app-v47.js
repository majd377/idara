...
function renderResidentDashboard(){
  const s=residentSelected();const periods=residentPeriodList();const p=selectedPeriod()||periods[0]||null;const t=p?currentTotals(p.id):null;
  if(p&&!state.periodId) state.periodId=p.id;
  setTitle('الرئيسية','اختر الأسبوع، وستتغير بيانات الأقسام الجانبية تلقائيًا حسب الأسبوع المختار.');
  $('#app').innerHTML=`<section class="welcome"><div class="welcome-copy"><div class="kicker">عمارة الأمين • حساب الساكن</div><h2>أهلاً ${safe(s?.name||'بالساكن')} 👋</h2><p class="muted">هذا الحساب للعرض فقط ولا يمكن تعديل البيانات.</p></div></section>
  <section class="panel resident-week-panel"><div class="panel-head"><div><h3>الأسبوع المعروض</h3><p class="muted">اختيارك هنا يطبق على المياه والكهرباء والمصاريف والمساهمات والدفعات وباقي بيانات العرض.</p></div><select id="residentHomeWeekSelect" class="period-select">${periods.map(x=>`<option value="${x.id}" ${sameId(x.id,p?.id)?'selected':''}>${safe(x.label||'أسبوع')} — ${fmtDate(x.startDate)} إلى ${fmtDate(x.endDate)}</option>`).join('')}</select></div></section>
  <section class="stats"><div class="stat"><div class="stat-label">سعر كيلو الكهرباء</div><div class="stat-value">${residentEnergyRateText()}</div><div class="stat-foot">للأسبوع المختار</div></div><div class="stat"><div class="stat-label">سعر كوب المياه</div><div class="stat-value">${t?.appliedPrice?waterPriceText(t.appliedPrice):'—'}</div><div class="stat-foot">للأسبوع المختار</div></div><div class="stat"><div class="stat-label">الأسبوع المختار</div><div class="stat-value">${p?fmtDate(p.startDate):'—'}</div><div class="stat-foot">${safe(p?.label||'—')}</div></div><div class="stat"><div class="stat-label">مديونيتك</div><div class="stat-value">${s?money(subscriberRow(s).debt):'—'}</div><div class="stat-foot">للعرض فقط</div></div></section>`;
  $('#residentHomeWeekSelect')?.addEventListener('change',e=>{state.periodId=e.target.value;renderResidentDashboard();});
}
  ...
  $('#saveReadings').onclick=async()=>{
    ...
    if(['accountant','operator'].includes(state.profile?.role) && !isCurrentPeriodId(pid)){
      ...
      for(const tr of rows){
        ...
        const manual=priceInput.dataset.manual==='1'?waterPrice(priceInput.value):null;
        const price=manual!=null?manual:weekPrice;
        const consumption=cur!==''&&prev!==''?Math.max(0,num(cur)-num(prev))/1000:null;
