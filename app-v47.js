...
function renderReadOnlyDashboard(){
  ...
  const price=p?currentTotals(p.id).appliedPrice:null;
...
function renderResidentWater(){
  ...
  const storedConsumption=r?.currentReading!=null&&r?.previousReading!=null?Math.max(0,num(r.currentReading)-num(r.previousReading))/1000:(r?.consumption!=null?num(r.consumption):null), storedPrice=r?((readingManualPrice(r)??(p?currentTotals(p.id).appliedPrice:null))):null, storedCharge=storedConsumption!=null&&storedPrice!=null?storedConsumption*storedPrice:null;
