...
app.post('/api/payments',(req,res)=>{try{const {subscriberId,amount,date,method='Cash',receiptNumber,reference,note}=req.body; const tx=db.transaction(()=>{const p=db.prepare(`INSERT INTO payments(subscriber_id,amount,payment_date,method,receipt_number,reference,note) VALUES (?,?,?,?,?,?,?)`).run(subscriberId,amount,date,method,receiptNumber||null,reference||null,note||null); db.prepare(`INSERT INTO ledger_transactions(subscriber_id,transaction_type,debit,credit,description,source_table,source_id) VALUES (?,?,0,?,?,?,?,?)`).run(subscriberId,'PAYMENT',amount,`دفعة رقم ${receiptNumber||p.lastInsertRowid}`,'payments',p.lastInsertRowid); return p.lastInsertRowid;}); const id=tx(); audit('CREATE','payment',id,null,req.body); res.json({id});}catch(e){res.status(400).json({error:e.message});}});
...
app.listen(port,()=>console.log(`Amin Building Manager running on http://localhost:${port}`));
