...
async function downloadResidentPdf(id,weekId,fromId,toId){
  ...
  clone.style.cssText='display:block;overflow:visible;width:1180px;max-width:none;background:#fff;';
  clone.querySelectorAll('*').forEach(el=>{el.style.maxWidth='none';});
  clone.querySelectorAll('table').forEach(t=>{t.style.width='100%';t.style.minWidth='0';});
  clone.querySelectorAll('table').forEach(t=>{t.style.borderCollapse='collapse';});
  const host=document.createElement('div');host.dir='rtl';
  host.style.cssText='position:absolute;left:-15000px;top:0;width:1220px;background:#fff;padding:28px;direction:rtl;font-family:Arial,sans-serif;color:#183734;z-index:99999;visibility:visible;';
...
function openAccount(id){
  ...
  openModal(`<section class="resident-report-card account-modal-report"><div class="resident-report-head"><div><span class="code">${safe(s.code)}</span><h2>${safe(s.name)}</h2><p>${safe(r.buildingName)} · ${safe(r.unitCode)} · ${safe(s.phone||'بدون هاتف')}</p></div><div class="balance-box"><span>المديونية</span><b>${money(r.debt)}</b></div></div><div class="account-section"><div class="section-head-inline"><div><h3>كشف الحساب الأسبوعي</h3><p>من الأقدم إلى الأحدث، وبنفس بنود التقرير.</p></div><span class="muted">${periods.length} أسبوع</span></div><div class="table-wrap resident-ledger-table"><table class="table" style="min-width:1250px"><thead><tr><th>الأسبوع</th><th>سحب المياه</th><th>قيمة المياه</th><th>خدمة الحارس</th><th>مصاريف أخرى</th><th>الديون السابقة</th><th>الإجمالي قبل الدفعات</th><th>الدفعات</th><th>الإجمالي المطلوب</th></tr></thead><tbody>${rows||`<tr><td colspan="9">${empty('لا يوجد كشف','')}</td></tr>`}</tbody></table></div></div></section>`);
}
function showGuide(){
  const steps=[
