...
const COLLECTIONS=['buildings','units','subscribers','meters','periods','readings','sources','energyReadings','costs','payments','ledger','members','waterSummary','seedDeletes','debts','contributions'];
...
async function ensureDefaults(){
  if(!can('admin'))return;
  // Show embedded historical data immediately so the UI never waits for hundreds of Firestore writes.
  // The full sync runs in the background after the first screen is rendered.
  mergeInitialDataIntoLocal();
  setTimeout(async()=>{
    try{
      await syncEmbeddedDefaults();
      window.__embeddedSyncDone = true;
      toast('تمت مزامنة البيانات التاريخية في الخلفية');
    }catch(e){
      console.warn('Embedded historical sync failed; local embedded data remains available.',e);
      window.__embeddedSyncError = e?.message || String(e);
      toast('تعذر مزامنة بعض البيانات التاريخية الآن، لكنها ظاهرة محليًا.','error');
    }
  }, 250);
  ...
  if(!(state.data.buildings||[]).length){
    ...
    state.loaded=false;await loadData(true);mergeInitialDataIntoLocal();
  }
}
...
function currentTotals(pid){
  ...
  const expenseCosts=cs.filter(c=>c.direction!=='credit' && c.type!=='مساهمة / خصم').reduce((a,c)=>a+num(c.amount),0);
  const legacyCredits=cs.filter(c=>c.direction==='credit' || c.type==='مساهمة / خصم').reduce((a,c)=>a+num(c.amount),0);
  const contributionCredits=(state.data.contributions||[]).filter(c=>c.periodId===pid).reduce((a,c)=>a+num(c.amount),0);
  const extras=expenseCosts - legacyCredits - contributionCredits;
...
async function renderReadings(){
  ...
  $('#newWeek2').onclick=showPeriodForm;$('#goCalc').onclick=()=>p?navigate('periods',p.id):showPeriodForm;if($('#firstWeek2'))$('#firstWeek2').onclick=showPeriodForm;$('#periodSelect')?.addEventListener('change',e=>navigate('readings',e.target.value));if(p){bindWaterSummaryInputs(p.id);bindReadingInputs(p.id);$$('[data-delete-reading]').forEach(b=>b.onclick=()=>deleteReading(b.dataset.deleteReading));} 
}
...
function readingTable(pid){
  ...
  const p=periodById(pid);return `<div class="table-wrap"><table class="table" style="min-width:980px"><thead><tr><th>الكود</th><th>الساكن</th><th>البناية / النوع</th><th>القراءة السابقة</th><th>القراءة الحالية</th><th>السحب</th><th>سعر الكوب</th><th>القيمة</th><th>الحالة</th><th></th></tr></thead><tbody>${rows.length?rows.map(x=>{const {s,r}=x;const cons=r.currentReading!=null&&r.previousReading!=null?Math.max(0,num(r.currentReading)-num(r.previousReading))/1000:null;const price=r.unitPrice??p.waterUnitPrice??'';const charge=cons!=null&&price!==''?cons*num(price):null;return `<tr data-rid="${r.id}"><td><span class="code">${safe(s.code)}</span></td><td><b>${safe(s.name)}</b></td><td>${s.type==='خارجي'?'<span class="badge warn">خارجي</span>':safe(subscriberRow(s).buildingName)}</td><td><input class="reading-input prev" type="number" step="0.001" value="${r.previousReading??''}"></td><td><input class="reading-input current" type="number" step="0.001" value="${r.currentReading??''}"></td><td class="cons">${cons==null?'—':fmt(cons,3)}</td><td><input class="reading-input price" type="number" step="0.01" value="${price}"></td><td class="charge">${charge==null?'—':money(charge)}</td><td class="status">${r.currentReading==null?'<span class="badge warn">بانتظار</span>':((num(r.currentReading)<num(r.previousReading))?'<span class="badge danger">تحقق</span>':'<span class="badge ok">جاهزة</span>')}</td><td><button class="mini red" data-delete-reading="${r.id}">حذف</button></td></tr>`}).join(''):`<tr><td colspan="10">${empty('لا توجد قراءات','تأكد من وجود سكان وعدادات قبل فتح الأسبوع.')}</td></tr>`}</tbody></table></div><div class="readings-actions"><span class="muted">حفظ القراءة لا يعتمد على حساب السعر؛ الحساب النهائي يتم من صفحة «الأسابيع والحساب».</span><button class="btn primary" id="saveReadings">حفظ كل القراءات</button></div>`;
}
async function deleteReading(id){if(!can('admin','manager','accountant','operator')){toast('لا تملك صلاحية الحذف','error');return;}if(!confirm('حذف قراءة المياه لهذا الساكن؟'))return;const r=(state.data.readings||[]).find(x=>x.id===id);if(!r)return;for(const l of (state.data.ledger||[]).filter(x=>x.referenceId===id)){if(l.transactionType==='WATER')await deleteDoc(orgDoc('ledger',l.id));}await deleteDoc(orgDoc('readings',id));removeLocal('readings',id);toast('تم حذف القراءة');renderReadings();}
...
async function deleteCost(id){
  if(!can('admin','manager','accountant')){toast('لا تملك صلاحية الحذف','error');return;} const c=(state.data.costs||[]).find(x=>x.id===id); if(!c)return;
  if(!confirm(`حذف المصروف «${c.description||c.type||'مصروف'}» بقيمة ${money(c.amount)}؟`))return;
  const batch=writeBatch(db); batch.delete(orgDoc('costs',id));
  for(const l of (state.data.ledger||[]).filter(x=>x.referenceId===id)) batch.delete(orgDoc('ledger',l.id));
...
function renderPayments(){
  ...
  $('#app').innerHTML=`<section class="panel"><div class="panel-head"><div><h2>الدفعات</h2><p>الدفع يسجل كحركة ائتمانية، والرصيد يبقى قابلًا للتتبع.</p></div><div class="panel-actions"><button class="btn soft" id="exportPayments">↓ Excel</button><button class="btn primary" id="newPay">+ تسجيل دفعة</button></div></div><div class="toolbar"><input id="paySearch" class="search" placeholder="ابحث باسم الساكن أو رقم الإيصال…"></div><div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>الساكن</th><th>المبلغ</th><th>الطريقة</th><th>الإيصال</th><th>الرصيد الحالي</th><th></th></tr></thead><tbody id="payBody">${paymentRows(pays)}</tbody></table></div></section>`;$('#newPay').onclick=showPaymentForm;$('#exportPayments').onclick=()=>exportPayments(pays);$$('[data-delete-payment]').forEach(b=>b.onclick=()=>deletePayment(b.dataset.deletePayment));$('#paySearch').oninput=e=>$('#payBody').innerHTML=paymentRows(pays.filter(p=>{const s=(state.data.subscribers||[]).find(x=>x.id===p.subscriberId);return [s?.name,p.receiptNumber].some(v=>String(v||'').includes(e.target.value))}));}
...
async function deletePayment(id){if(!can('admin','manager','accountant')){toast('لا تملك صلاحية الحذف','error');return;}const p=(state.data.payments||[]).find(x=>x.id===id);if(!p)return;if(!confirm(`حذف الدفعة ${money(p.amount)}؟`))return;const batch=writeBatch(db);batch.delete(orgDoc('payments',id));for(const l of (state.data.ledger||[]).filter(x=>x.referenceId===id))batch.delete(orgDoc('ledger',l.id));await batch.commit();state.loaded=false;await loadData(true);toast('تم حذف الدفعة');renderPayments();}
...
function renderContributions(){
  setTitle('المساهمات والخصومات','هذه الصفحة للخصومات التي تقلل تكلفة تشغيل المياه. لا تسجل المساهمة داخل المصاريف.');
  const periods=latestPeriods(); if(!state.periodId) state.periodId=periods[0]?.id; const p=selectedPeriod(); const rows=p?(state.data.contributions||[]).filter(x=>x.periodId===p.id):[];
  $('#app').innerHTML=`<section class="panel"><div class="panel-head"><div><h2>المساهمات والخصومات</h2><p>أي مبلغ مساهمة يقلل التكلفة قبل حساب سعر الكوب.</p></div><button class="btn primary" id="addContribution">+ إضافة مساهمة / خصم</button></div>${!p?empty('افتح أسبوعًا أولًا','المساهمات مرتبطة بالأسبوع.'): `<div class="period-picker"><label>الأسبوع الحالي</label><select id="periodSelect" class="period-select">${periods.map(x=>`<option value="${x.id}" ${x.id===p.id?'selected':''}>${safe(x.label||'أسبوع')} — ${fmtDate(x.startDate)} إلى ${fmtDate(x.endDate)}</option>`).join('')}</select></div><div class="money-grid"><div class="money-card"><small>إجمالي المساهمات / الخصومات</small><b>${money(rows.reduce((a,x)=>a+num(x.amount),0))}</b></div><div class="money-card"><small>عدد البنود</small><b>${rows.length}</b></div></div><div class="section-note" style="margin-top:13px"><b>مثال:</b> إذا تكلفة التشغيل 1,758 ₪ وفي مساهمة 200 ₪، تصبح تكلفة التشغيل الصافية 1,558 ₪ قبل حساب سعر الكوب.</div><div class="table-wrap" style="margin-top:13px"><table class="table"><thead><tr><th>التاريخ</th><th>البيان</th><th>المبلغ</th><th></th></tr></thead><tbody>${rows.length?rows.map(c=>`<tr><td>${fmtDate(c.date)}</td><td>${safe(c.description||'مساهمة')}</td><td class="strong">${money(c.amount)}</td><td><button class="mini red" data-delete-contribution="${c.id}">حذف</button></td></tr>`).join(''):`<tr><td colspan="4">${empty('لا توجد مساهمات','أضف خصمًا أو مساهمة من الزر أعلاه.')}</td></tr>`}</tbody></table></div>`}</section>`;
  $('#addContribution').onclick=()=>showContributionForm(p?.id);$('#periodSelect')?.addEventListener('change',e=>navigate('contributions',e.target.value));$$('[data-delete-contribution]').forEach(b=>b.onclick=()=>deleteContribution(b.dataset.deleteContribution));
}
function showContributionForm(pid,id){
  if(!can('admin','manager','accountant')){toast('إضافة المساهمات مخصصة للإدارة والمحاسبة','error');return;}
  const c=id?(state.data.contributions||[]).find(x=>x.id===id):null;
  openModal(`<h2>${c?'تعديل مساهمة / خصم':'إضافة مساهمة / خصم'}</h2><p class="modal-lead">المبلغ هنا يُخصم من تكلفة تشغيل المياه ولا يُحمّل على السكان كرسوم مستقلة.</p><div class="form-grid"><div class="field"><label>المبلغ</label><input id="dAmount" type="number" min="0" step="0.01" value="${c?.amount??''}"></div><div class="field"><label>التاريخ</label><input id="dDate" type="date" value="${c?.date||dateNow()}"></div><div class="field full"><label>البيان</label><input id="dDesc" value="${safe(c?.description||'')}" placeholder="مثال: مساهمة سولار"></div></div><div class="actions"><button class="btn primary" id="saveContribution">حفظ</button><button class="btn ghost" id="cancelContribution">إلغاء</button></div>`);
  $('#cancelContribution').onclick=closeModal;$('#saveContribution').onclick=async()=>{const amount=num($('#dAmount').value);if(amount<=0){toast('اكتب مبلغًا صحيحًا','error');return;}const data={periodId:pid,amount,date:$('#dDate').value,description:$('#dDesc').value.trim()||'مساهمة / خصم',createdBy:c?.createdBy||state.user.uid,updatedAt:serverTimestamp()};if(c)await updateDoc(orgDoc('contributions',id),data);else{const r=doc(orgCollection('contributions'));await setDoc(r,{...data,createdAt:serverTimestamp()});}state.loaded=false;await loadData(true);closeModal();toast(c?'تم تعديل المساهمة':'تمت إضافة المساهمة');renderContributions();};
}
async function deleteContribution(id){if(!can('admin','manager','accountant')){toast('لا تملك صلاحية الحذف','error');return;}const c=(state.data.contributions||[]).find(x=>x.id===id);if(!c)return;if(!confirm(`حذف مساهمة بقيمة ${money(c.amount)}؟`))return;await deleteDoc(orgDoc('contributions',id));removeLocal('contributions',id);toast('تم حذف المساهمة');renderContributions();}
...
function subscriberFinancialSummary(id,periodId=null){
  ...
  const before=p?ledger.filter(x=>{const per=periodById(x.periodId);return !x.periodId || !per || String(per.startDate)<String(p.startDate);}):ledger;
  // The previous balance is the already outstanding amount before the selected week.
  const previousGross=before.reduce((a,x)=>a+num(x.debit),0);
  const previousPayments=before.filter(x=>x.transactionType==='PAYMENT').reduce((a,x)=>a+num(x.credit),0);
  const previousBalance=Math.max(0,previousGross-previousPayments);
  const currentWater=current.filter(x=>x.transactionType==='WATER').reduce((a,x)=>a+num(x.debit),0);
  const currentWaterReading=(state.data.readings||[]).find(r=>{const m=(state.data.meters||[]).find(m=>m.id===r.meterId);return m?.subscriberId===id&&r.periodId===p?.id;});
  const currentWaterConsumption=currentWaterReading?.consumption??0;
  ...
  const currentGross=currentWater+services+debtsCurrent+other;
  // Payments are shown separately and deducted only once at the final total.
  const finalBalance=Math.max(0,previousBalance+currentGross-periodPayments);
  const debtRecords=(state.data.debts||[]).filter(d=>d.subscriberId===id);
  const unpaidDebts=debtRecords.reduce((a,d)=>a+Math.max(0,num(d.amount)-num(d.paidAmount)),0);
  return {s,p,previousBalance,currentWater,currentWaterConsumption,services,debtsCurrent,other,periodPayments,totalPayments,unpaidDebts,finalBalance,currentGross};
}
...
async function cleanupOldPeriodsKeep0903(){
  if(!can('admin')){toast('تنظيف الأسابيع مخصص للمدير','error');return;}
  const keep=(state.data.periods||[]).find(p=>String(p.startDate)==='2026-09-03');
  if(!keep){toast('أسبوع 03/09/2026 غير موجود.','error');return;}
  const old=(state.data.periods||[]).filter(p=>p.id!==keep.id);
  if(!old.length){toast('لا توجد أسابيع قديمة للحذف.');return;}
  if(!confirm(`سيتم حذف ${old.length} أسبوعًا وكل القراءات والكهرباء والمصاريف والحركات المرتبطة بها، والإبقاء فقط على أسبوع 03/09/2026.\n\nهل أنت متأكد؟`))return;
  const ops=[];
  for(const p of old){ops.push(batch=>batch.delete(orgDoc('periods',p.id)));for(const c of ['readings','energyReadings','costs','waterSummary','contributions'])for(const r of (state.data[c]||[]).filter(x=>x.periodId===p.id))ops.push(batch=>batch.delete(orgDoc(c,r.id)));for(const l of (state.data.ledger||[]).filter(x=>x.periodId===p.id))ops.push(batch=>batch.delete(orgDoc('ledger',l.id));)}
  const deletedSeeds=old.map(p=>{const seed=INITIAL_DATA.periods.find(x=>x.id===p.id);return seed?seedKeyForCleanup('periods',seed.id):null;}).filter(Boolean);
  for(const key of deletedSeeds)ops.push(batch=>batch.set(orgDoc('seedDeletes',`cleanup_${key.replace('/','__')}`),{key,deletedAt:serverTimestamp(),deletedBy:state.user.uid}));
  await commitOps(ops);state.loaded=false;await loadData(true);state.periodId=keep.id;toast('تم حذف الأسابيع القديمة والإبقاء على 03/09/2026');renderSettings();
}
function seedKeyForCleanup(c,id){return `${c}/${id}`;}
