...
import {
  ...
  addDoc, updateDoc, deleteDoc, writeBatch, serverTimestamp, ADMIN_EMAILS
} from './firebase-init-v31.js';

const $ = s => document.querySelector(s);
const $$ = s => [...document.querySelectorAll(s)];
const state = {user:null,profile:null,view:'dashboard',periodId:null,fundMonth:new Date().toISOString().slice(0,7),fundLoaded:false,fundLoading:false,data:{},loaded:false,loading:false};

function header(title,subtitle,actions=''){return `<div class="panel-head"><div><h3>${safe(title)}</h3><p>${safe(subtitle||'')}</p></div><div class="panel-actions">${actions}</div></div>`;}
...
const VIEW_NAMES={dashboard:'الرئيسية',periods:'الأسابيع والحساب',readings:'قراءات الماء',energy:'الكهرباء والمولدات',costs:'المصاريف والطوارئ',guard:'خدمة الحارس',contributions:'المساهمات والخصومات',subscribers:'السكان والوحدات',payments:'الدفعات والأرصدة',debts:'الديون السابقة',reports:'التقارير والتصدير',fund:'صندوق العمارة والإيرادات',settings:'الإعدادات والصلاحيات',guide:'دليل استخدام عملي',historical:'البيانات التاريخية'};
...
async function ensureProfile(){
  ...
  const isAdmin=ADMIN_EMAILS.map(x=>x.toLowerCase()).includes((state.user.email||'').toLowerCase());const role=isAdmin?'admin':'pending';
...
const FUND_COLLECTIONS=['fundGuardMonths','fundGuardPayments','fundGuardExpenses','solarReceipts','solarSales','fundOtherIncome','fundExpenses','fundConfig'];
async function loadFundData(force=false){
  if(state.fundLoaded&&!force)return;
  if(state.fundLoading)return;
  state.fundLoading=true;
  try{
    const results=await Promise.all(FUND_COLLECTIONS.map(async c=>{const snap=await getDocs(orgCollection(c));return [c,snap.docs.map(d=>({id:d.id,...d.data()}))]}));
    for(const [c,rows] of results) state.data[c]=rows;
    state.fundLoaded=true;
  }finally{state.fundLoading=false;}
}
function fundRows(c){return state.data[c]||[]}
function fundMonthLabel(v){const [y,m]=String(v||'').split('-');const names=['يناير','فبراير','مارس','أبريل','مايو','يونيو','يوليو','أغسطس','سبتمبر','أكتوبر','نوفمبر','ديسمبر'];return y&&m?`${names[Number(m)-1]||m} ${y}`:v||'—'}
function fundMoney(v){return `${new Intl.NumberFormat('en-US',{maximumFractionDigits:0}).format(Math.round(Number(v||0)))} ₪`}
function fundQty(v){return new Intl.NumberFormat('en-US',{maximumFractionDigits:3}).format(Number(v||0))}
function fundSubscribers(){return (state.data.subscribers||[]).filter(s=>s.active!==false&&String(s.type||'').toLowerCase()!=='خارجي').sort((a,b)=>String(a.code||'').localeCompare(String(b.code||''),undefined,{numeric:true}));}
function fundGuardMonth(){return fundRows('fundGuardMonths').find(x=>x.month===state.fundMonth)}
function fundGuardFee(){return Number(fundGuardMonth()?.feePerOwner ?? 30)}
function fundGuardSalary(){return Number(fundGuardMonth()?.salary ?? 800)}
function fundGuardPayMap(){return new Map(fundRows('fundGuardPayments').filter(x=>x.month===state.fundMonth).map(x=>[x.subscriberId,x]))}
function fundGuardItems(){const map=fundGuardPayMap();return fundSubscribers().map(s=>{const r=map.get(s.id)||{};const included=r.included!==false;const due=included?fundGuardFee():0;const paid=Number(r.paidAmount||0);const remaining=Math.max(0,due-paid);const status=due===0?'غير مشمول':paid<=0?'غير مدفوع':paid>=due?'مدفوع':'مدفوع جزئيًا';return {s,included,due,paid,remaining,status,r}})}
function fundGuardSummary(){const items=fundGuardItems();const gm=fundGuardMonth();const required=Number(gm?.manualRevenue??0),collected=items.reduce((a,x)=>a+x.paid,0),clean=fundRows('fundGuardExpenses').filter(x=>String(x.month||state.fundMonth)===state.fundMonth).reduce((a,x)=>a+Number(x.amount||0),0),other=fundRows('fundOtherIncome').filter(x=>String(x.date||'').startsWith(state.fundMonth)&&x.destination==='guard').reduce((a,x)=>a+Number(x.amount||0),0),salary=fundGuardSalary();return {items,required,collected,uncollected:required>0?Math.max(0,required-collected):0,clean,other,salary,net:collected+other-salary-clean}}
function fundSolarRows(c){return fundRows(c).filter(x=>String(x.date||'').startsWith(state.fundMonth)).sort((a,b)=>String(a.date||'').localeCompare(String(b.date||'')))}
function fundSolarSummary(){const rec=fundSolarRows('solarReceipts'),sales=fundSolarRows('solarSales');const received=rec.reduce((a,x)=>a+Number(x.liters||0),0),sold=sales.reduce((a,x)=>a+Number(x.liters||0),0),salesValue=sales.reduce((a,x)=>a+(x.totalAmount!=null?Number(x.totalAmount):Number(x.liters||0)*Number(x.pricePerLiter||0)),0),transport=rec.reduce((a,x)=>a+Math.max(10,Number(x.transportCost||10)),0);return {rec,sales,received,sold,remaining:received-sold,salesValue,transport,net:salesValue-transport}}
function fundOpening(){return Number(fundRows('fundConfig').find(x=>x.id==='main')?.initialBalance||0)}
function fundFundSummary(){const [y,m]=String(state.fundMonth).split('-').map(Number);const start=`${state.fundMonth}-01`;const dt=new Date(y,m-1,1);const before=new Date(dt);const solar=fundSolarSummary();let opening=fundOpening();for(const r of fundRows('solarSales')){if(String(r.date||'')<start) opening+=Number(r.totalAmount!=null?r.totalAmount:Number(r.liters||0)*Number(r.pricePerLiter||0));}for(const r of fundRows('solarReceipts')){if(String(r.date||'')<start) opening-=Math.max(10,Number(r.transportCost||10));}for(const r of fundRows('fundOtherIncome')){if(String(r.date||'')<start&&r.destination==='fund') opening+=Number(r.amount||0);}for(const r of fundRows('fundExpenses')){if(String(r.date||'')<start) opening-=Number(r.amount||0);}const other=fundRows('fundOtherIncome').filter(x=>String(x.date||'').startsWith(state.fundMonth)&&x.destination==='fund').reduce((a,x)=>a+Number(x.amount||0),0);const expenses=fundRows('fundExpenses').filter(x=>String(x.date||'').startsWith(state.fundMonth)).reduce((a,x)=>a+Number(x.amount||0),0);return {opening,solarNet:solar.net,other,expenses,closing:opening+solar.net+other-expenses}}
function fundGuardMonthForm(){const gm=fundGuardMonth();openModal(`<h2>${gm?'تعديل':'إعداد'} خدمة الحارس</h2><div class="form-grid"><div class="field"><label>الشهر</label><input id="fgMonth" type="month" value="${safe(state.fundMonth)}"></div><div class="field"><label>المبلغ على الشخص الواحد</label><input id="fgFee" class="ltr" type="number" min="0" step="1" value="${fundGuardFee()}"></div><div class="field"><label>إجمالي إيراد خدمة الحارس الفعلي (تدخله أنت يدويًا)</label><input id="fgRevenue" class="ltr" type="number" min="0" step="1" value="${Number(gm?.manualRevenue||0)}"></div><div class="field"><label>راتب الحارس</label><input id="fgSalary" class="ltr" type="number" min="0" step="1" value="${fundGuardSalary()}"></div></div><div class="hint">المبلغ على الشخص يُستخدم فقط لمعرفة مستحق كل مالك. الإيراد الفعلي لا يحسبه النظام تلقائيًا: أنت تدخل المبلغ الذي تم تحصيله فعليًا. مدفوعات السكان تبقى سجلات مستقلة.</div><div class="actions"><button class="btn primary" id="saveFG">حفظ</button></div>`);$('#saveFG').onclick=async()=>{if(!can('admin','manager','accountant')){toast('لا تملك صلاحية التعديل','error');return;}const month=$('#fgMonth').value||state.fundMonth;const data={id:gm?.id||`guard-${month}`,month,feePerOwner:num($('#fgFee').value),manualRevenue:num($('#fgRevenue').value),salary:num($('#fgSalary').value)};await saveDoc('fundGuardMonths',gm?.id||data.id,data,true);state.fundLoaded=false;state.fundMonth=month;await loadFundData(true);closeModal();render();}}
function fundGuardExpenseForm(id=null){const r=id?fundRows('fundGuardExpenses').find(x=>x.id===id):null;openModal(`<h2>${r?'تعديل':'إضافة'} مصروف نظافة</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="fgeDate" type="date" value="${r?.date||`${state.fundMonth}-01`}"></div><div class="field"><label>اسم المصروف</label><input id="fgeDesc" value="${safe(r?.description||'')}" placeholder="قشاطة، مكنسة، كلور..."></div><div class="field"><label>المبلغ</label><input id="fgeAmount" class="ltr" type="number" min="0" step="1" value="${num(r?.amount)}"></div><div class="field full"><label>ملاحظات</label><textarea id="fgeNotes">${safe(r?.notes||'')}</textarea></div></div><div class="actions"><button class="btn primary" id="saveFGE">حفظ</button></div>`);$('#saveFGE').onclick=async()=>{const date=$('#fgeDate').value;const data={month:date.slice(0,7),date,description:$('#fgeDesc').value.trim()||'مصروف نظافة',amount:num($('#fgeAmount').value),type:'cleaning',notes:$('#fgeNotes').value.trim()};if(id)await saveDoc('fundGuardExpenses',id,data);else await addRow('fundGuardExpenses',data);state.fundLoaded=false;state.fundMonth=data.month;await loadFundData(true);closeModal();render();}}
async function fundSaveGuardPayments(){if(!can('admin','manager','accountant')){toast('لا تملك صلاحية الحفظ','error');return;}const batch=writeBatch(db);let count=0;for(const item of fundGuardItems()){const inc=$(`[data-fund-include="${item.s.id}"]`);const paid=$(`[data-fund-paid="${item.s.id}"]`);if(!inc||!paid)continue;const ref=item.r?.id?orgDoc('fundGuardPayments',item.r.id):doc(orgCollection('fundGuardPayments'));batch.set(ref,{id:ref.id,month:state.fundMonth,subscriberId:item.s.id,included:inc.checked,paidAmount:num(paid.value),updatedAt:serverTimestamp(),updatedBy:state.user.uid},{merge:true});count++;}await batch.commit();state.fundLoaded=false;await loadFundData(true);toast(`تم حفظ ${count} سجل للحارس`);render();}
function fundSolarReceiptForm(id=null){const r=id?fundRows('solarReceipts').find(x=>x.id===id):null;openModal(`<h2>${r?'تعديل':'إضافة'} استلام سولار</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="srDate" type="date" value="${r?.date||`${state.fundMonth}-01`}"></div><div class="field"><label>الكمية باللتر</label><input id="srLiters" class="ltr" type="number" min="0" step="0.001" value="${num(r?.liters)}"></div><div class="field"><label>المصدر</label><input id="srSource" value="${safe(r?.source||'بلدية')}"></div><div class="field"><label>تكلفة النقل</label><input id="srTransport" class="ltr" type="number" min="10" step="1" value="${Math.max(10,num(r?.transportCost||10))}"></div><div class="field full"><label>ملاحظات</label><textarea id="srNotes">${safe(r?.notes||'')}</textarea></div></div><div class="actions"><button class="btn primary" id="saveSR">حفظ</button></div>`);$('#saveSR').onclick=async()=>{const date=$('#srDate').value;const data={date,liters:num($('#srLiters').value),source:$('#srSource').value.trim(),transportCost:Math.max(10,num($('#srTransport').value)),notes:$('#srNotes').value.trim()};if(id)await saveDoc('solarReceipts',id,data);else await addRow('solarReceipts',data);state.fundLoaded=false;state.fundMonth=date.slice(0,7);await loadFundData(true);closeModal();render();}}
function fundSolarSaleForm(id=null){const r=id?fundRows('solarSales').find(x=>x.id===id):null;openModal(`<h2>${r?'تعديل':'إضافة'} بيع سولار</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="ssDate" type="date" value="${r?.date||`${state.fundMonth}-01`}"></div><div class="field"><label>اللترات المباعة</label><input id="ssLiters" class="ltr" type="number" min="0" step="0.001" value="${num(r?.liters)}"></div><div class="field"><label>سعر اللتر</label><input id="ssPrice" class="ltr" type="number" min="0" step="1" value="${num(r?.pricePerLiter)}"></div><div class="field"><label>الإجمالي المباشر (اختياري)</label><input id="ssTotal" class="ltr" type="number" min="0" step="1" value="${r?.totalAmount!=null?num(r.totalAmount):''}"></div><div class="field"><label>المشتري (اختياري)</label><input id="ssBuyer" value="${safe(r?.buyer||'')}"></div><div class="field full"><label>ملاحظات</label><textarea id="ssNotes">${safe(r?.notes||'')}</textarea></div></div><div class="notice">الإجمالي = اللترات × سعر اللتر، أو الإجمالي المباشر إذا كتبته.</div><div class="actions"><button class="btn primary" id="saveSS">حفظ</button></div>`);$('#saveSS').onclick=async()=>{const date=$('#ssDate').value;const total=$('#ssTotal').value.trim();const data={date,liters:num($('#ssLiters').value),pricePerLiter:num($('#ssPrice').value),buyer:$('#ssBuyer').value.trim(),notes:$('#ssNotes').value.trim()};if(total)data.totalAmount=num(total);if(id)await saveDoc('solarSales',id,data);else await addRow('solarSales',data);state.fundLoaded=false;state.fundMonth=date.slice(0,7);await loadFundData(true);closeModal();render();}}
function fundOtherIncomeForm(id=null){const r=id?fundRows('fundOtherIncome').find(x=>x.id===id):null;openModal(`<h2>${r?'تعديل':'إضافة'} إيراد آخر</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="oiDate" type="date" value="${r?.date||`${state.fundMonth}-01`}"></div><div class="field"><label>اسم/نوع الإيراد</label><input id="oiName" value="${safe(r?.name||'')}"></div><div class="field"><label>المبلغ</label><input id="oiAmount" class="ltr" type="number" min="0" step="1" value="${num(r?.amount)}"></div><div class="field"><label>المصدر</label><input id="oiSource" value="${safe(r?.source||'')}"></div><div class="field"><label>مكان إضافة الإيراد</label><select id="oiDest"><option value="fund" ${r?.destination!=='guard'?'selected':''}>صندوق العمارة</option><option value="guard" ${r?.destination==='guard'?'selected':''}>حساب خدمات الحارس</option></select></div><div class="field full"><label>ملاحظات</label><textarea id="oiNotes">${safe(r?.notes||'')}</textarea></div></div><div class="actions"><button class="btn primary" id="saveOI">حفظ</button></div>`);$('#saveOI').onclick=async()=>{const date=$('#oiDate').value;const data={date,name:$('#oiName').value.trim(),amount:num($('#oiAmount').value),source:$('#oiSource').value.trim(),destination:$('#oiDest').value,notes:$('#oiNotes').value.trim()};if(id)await saveDoc('fundOtherIncome',id,data);else await addRow('fundOtherIncome',data);state.fundLoaded=false;state.fundMonth=date.slice(0,7);await loadFundData(true);closeModal();render();}}
function fundExpenseForm(id=null){const r=id?fundRows('fundExpenses').find(x=>x.id===id):null;openModal(`<h2>${r?'تعديل':'إضافة'} مصروف من الصندوق</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="fxDate" type="date" value="${r?.date||`${state.fundMonth}-01`}"></div><div class="field"><label>نوع المصروف</label><select id="fxType"><option>صيانة</option><option>طوارئ</option><option>أصول العمارة</option><option>مرافق مشتركة</option><option>أخرى</option></select></div><div class="field"><label>المبلغ</label><input id="fxAmount" class="ltr" type="number" min="0" step="1" value="${num(r?.amount)}"></div><div class="field"><label>المستفيد</label><input id="fxPayee" value="${safe(r?.payee||'')}"></div><div class="field full"><label>الوصف</label><input id="fxDesc" value="${safe(r?.description||'')}"></div><div class="field full"><label>رابط صورة الفاتورة (اختياري)</label><input id="fxAttach" class="ltr" value="${safe(r?.attachmentUrl||'')}"></div><div class="field full"><label>ملاحظات</label><textarea id="fxNotes">${safe(r?.notes||'')}</textarea></div></div><div class="actions"><button class="btn primary" id="saveFX">حفظ</button></div>`);if(r)$('#fxType').value=r.type||'أخرى';$('#saveFX').onclick=async()=>{const date=$('#fxDate').value;const data={date,type:$('#fxType').value,amount:num($('#fxAmount').value),payee:$('#fxPayee').value.trim(),description:$('#fxDesc').value.trim(),attachmentUrl:$('#fxAttach').value.trim(),notes:$('#fxNotes').value.trim()};if(id)await saveDoc('fundExpenses',id,data);else await addRow('fundExpenses',data);state.fundLoaded=false;state.fundMonth=date.slice(0,7);await loadFundData(true);closeModal();render();}}
function fundOpeningForm(){const cur=fundOpening();openModal(`<h2>الرصيد الافتتاحي لصندوق العمارة</h2><div class="field"><label>الرصيد</label><input id="foAmount" class="ltr" type="number" step="1" value="${cur}"></div><div class="actions"><button class="btn primary" id="saveFO">حفظ</button></div>`);$('#saveFO').onclick=async()=>{if(!can('admin','manager','accountant')){toast('لا تملك صلاحية التعديل','error');return;}await setDoc(orgDoc('fundConfig','main'),{id:'main',initialBalance:num($('#foAmount').value),updatedAt:serverTimestamp(),updatedBy:state.user.uid},{merge:true});state.fundLoaded=false;await loadFundData(true);closeModal();render();}}
async function fundDeleteDoc(c,id,label){if(!can('admin','manager')){toast('الحذف للمديرين فقط','error');return;}if(!confirm(`هل تريد حذف ${label||'هذا السجل'}؟`))return;await deleteDoc(orgDoc(c,id));state.fundLoaded=false;await loadFundData(true);toast('تم الحذف');render();}
function renderFund(){
  setTitle('صندوق العمارة والإيرادات','قسم مستقل داخل نفس النظام، لا يدخل في حسبة المياه أو سعر الكوب.');
  if(!state.fundLoaded){$('#app').innerHTML='<section class="panel fund-loading"><div class="fund-spinner"></div><h2>جاري فتح صندوق العمارة…</h2><p class="muted">تحميل بيانات الصندوق فقط.</p></section>';loadFundData().then(()=>render()).catch(e=>{$('#app').innerHTML=`<section class="panel"><h2>تعذر فتح صندوق العمارة</h2><p class="muted">${safe(e?.message||'تعذر تحميل بيانات الصندوق')}</p></section>`});return;}
  const g=fundGuardSummary(),s=fundSolarSummary(),f=fundFundSummary();
  const tabs=[['overview','نظرة عامة'],['guard','خدمة الحارس'],['solar','السولار'],['fund','صندوق العمارة'],['other','إيرادات أخرى']];
  let html=`<section class="fund-hero"><div><div class="eyebrow">${fundMonthLabel(state.fundMonth)} · مستقل عن حسبة المياه</div><h2>صندوق العمارة والإيرادات</h2><p>هنا ندير خدمة الحارس، السولار، الإيرادات الأخرى، ومصروفات الصندوق بدون أي تأثير على سعر الكوب.</p></div><div class="fund-month"><label>الشهر</label><input id="fundMonthPicker" type="month" value="${safe(state.fundMonth)}"></div></section>`;
  html+=`<div class="fund-tabs">${tabs.map(([k,t])=>`<button class="fund-tab ${state.fundTab===k?'active':''}" data-fund-tab="${k}">${t}</button>`).join('')}</div>`;
  html+=`<section class="fund-pane">`;
  const tab=state.fundTab||'overview';
  if(tab==='overview') html+=`<div class="fund-kpis"><div class="fund-kpi"><span>الإيراد المحدد يدويًا</span><b>${g.required?fundMoney(g.required):'غير محدد'}</b></div><div class="fund-kpi"><span>المحصل فعليًا</span><b>${fundMoney(g.collected)}</b></div><div class="fund-kpi"><span>صافي حساب الحارس</span><b>${fundMoney(g.net)}</b></div><div class="fund-kpi"><span>السولار المستلم</span><b>${fundQty(s.received)} لتر</b></div><div class="fund-kpi"><span>السولار المباع</span><b>${fundQty(s.sold)} لتر</b></div><div class="fund-kpi"><span>قيمة المبيعات</span><b>${fundMoney(s.salesValue)}</b></div><div class="fund-kpi"><span>نقل السولار</span><b>${fundMoney(s.transport)}</b></div><div class="fund-kpi"><span>رصيد صندوق العمارة</span><b>${fundMoney(f.closing)}</b></div></div><div class="grid-2"><div class="panel"><h3>حساب خدمات الحارس</h3><div class="fund-lines"><div><span>الإيراد المحدد يدويًا</span><b>${g.required?fundMoney(g.required):'غير محدد'}</b></div><div><span>المحصل فعليًا</span><b>${fundMoney(g.collected)}</b></div><div><span>المتبقي من الهدف</span><b>${g.required?fundMoney(g.uncollected):'غير محدد'}</b></div><div><span>الراتب</span><b>${fundMoney(g.salary)}</b></div><div><span>النظافة</span><b>${fundMoney(g.clean)}</b></div><div><span>الصافي</span><b>${fundMoney(g.net)}</b></div></div></div><div class="panel"><h3>صندوق العمارة</h3><div class="fund-lines"><div><span>رصيد البداية</span><b>${fundMoney(f.opening)}</b></div><div><span>صافي السولار الداخل للصندوق</span><b>${fundMoney(f.solarNet)}</b></div><div><span>إيرادات أخرى</span><b>${fundMoney(f.other)}</b></div><div><span>مصروفات الصندوق</span><b>${fundMoney(f.expenses)}</b></div><div class="highlight"><span>الرصيد الحالي</span><b>${fundMoney(f.closing)}</b></div></div></div></div>`;
  if(tab==='guard') html+=`<div class="panel">${header('خدمة الحارس','المبلغ على الشخص للتوزيع والمتابعة، أما إجمالي الإيراد الفعلي فتسجله أنت يدويًا. ويمكنك استثناء من لا يدفع.','<div class="panel-actions"><button class="btn primary" data-fund-action="guard-settings">إعداد الشهر</button><button class="btn soft" data-fund-action="save-guard">حفظ الدفعات</button></div>')}<div class="fund-kpis"><div class="fund-kpi"><span>المبلغ للشخص</span><b>${fundMoney(fundGuardFee())}</b></div><div class="fund-kpi"><span>المشمولون</span><b>${g.items.filter(x=>x.included).length}</b></div><div class="fund-kpi"><span>المحصل</span><b>${fundMoney(g.collected)}</b></div><div class="fund-kpi"><span>راتب الحارس</span><b>${fundMoney(g.salary)}</b></div></div><div class="table-wrap"><table class="table"><thead><tr><th>الكود</th><th>الاسم</th><th>البناية</th><th>مشمول بالحارس</th><th>المطلوب</th><th>المدفوع</th><th>المتبقي</th><th>الحالة</th></tr></thead><tbody>${g.items.map(x=>`<tr><td class="ltr">${safe(x.s.code||'')}</td><td>${safe(x.s.name||'')}</td><td>${safe(buildingName(x.s.unitId))}</td><td><input type="checkbox" data-fund-include="${x.s.id}" ${x.included?'checked':''}></td><td>${fundMoney(x.due)}</td><td><input class="ltr reading-input" type="number" min="0" step="1" value="${x.paid}" data-fund-paid="${x.s.id}"></td><td>${fundMoney(x.remaining)}</td><td>${x.status}</td></tr>`).join('')}</tbody></table></div><div class="mini-section"><div class="panel-head"><div><h3>مصاريف النظافة</h3><p>أدوات النظافة الخاصة بخدمة الحارس.</p></div><button class="btn primary" data-fund-action="add-clean">+ مصروف نظافة</button></div><div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>المصروف</th><th>المبلغ</th><th>ملاحظات</th><th></th></tr></thead><tbody>${fundRows('fundGuardExpenses').filter(x=>String(x.date||'').startsWith(state.fundMonth)).map(r=>`<tr><td>${fmtDate(r.date)}</td><td>${safe(r.description||'')}</td><td>${fundMoney(r.amount)}</td><td>${safe(r.notes||'')}</td><td><button class="btn small ghost" data-fund-action="edit-clean" data-id="${r.id}">تعديل</button><button class="btn small danger" data-fund-action="del" data-c="fundGuardExpenses" data-id="${r.id}">حذف</button></td></tr>`).join('')||`<tr><td colspan="5">${empty('لا توجد مصاريف نظافة')}</td></tr>`}</tbody></table></div></div></div>`;
  if(tab==='solar') html+=`<div class="grid-2"><div class="panel">${header('استلام السولار','كل عملية استلام مستقلة، والنقل لا يقل عن 10 ₪.','<button class="btn primary" data-fund-action="add-receipt">+ استلام سولار</button>')}<div class="fund-lines"><div><span>إجمالي المستلم</span><b>${fundQty(s.received)} لتر</b></div><div><span>إجمالي النقل</span><b>${fundMoney(s.transport)}</b></div><div><span>المتبقي</span><b>${fundQty(s.remaining)} لتر</b></div></div><div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>اللترات</th><th>المصدر</th><th>النقل</th><th></th></tr></thead><tbody>${s.rec.map(r=>`<tr><td>${fmtDate(r.date)}</td><td class="ltr">${fundQty(r.liters)}</td><td>${safe(r.source||'')}</td><td>${fundMoney(r.transportCost||10)}</td><td><button class="btn small ghost" data-fund-action="edit-receipt" data-id="${r.id}">تعديل</button><button class="btn small danger" data-fund-action="del" data-c="solarReceipts" data-id="${r.id}">حذف</button></td></tr>`).join('')||`<tr><td colspan="5">${empty('لا توجد عمليات استلام')}</td></tr>`}</tbody></table></div></div><div class="panel">${header('بيع السولار','سعر البيع حسب السوق، ويمكن إدخال الإجمالي مباشرة.','<button class="btn primary" data-fund-action="add-sale">+ بيع سولار</button>')}<div class="fund-lines"><div><span>المباع</span><b>${fundQty(s.sold)} لتر</b></div><div><span>قيمة المبيعات</span><b>${fundMoney(s.salesValue)}</b></div><div class="highlight"><span>الصافي للصندوق</span><b>${fundMoney(s.net)}</b></div></div><div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>اللترات</th><th>سعر اللتر</th><th>الإجمالي</th><th>المشتري</th><th></th></tr></thead><tbody>${s.sales.map(r=>`<tr><td>${fmtDate(r.date)}</td><td class="ltr">${fundQty(r.liters)}</td><td>${fundMoney(r.pricePerLiter)}</td><td>${fundMoney(r.totalAmount!=null?r.totalAmount:num(r.liters)*num(r.pricePerLiter))}</td><td>${safe(r.buyer||'')}</td><td><button class="btn small ghost" data-fund-action="edit-sale" data-id="${r.id}">تعديل</button><button class="btn small danger" data-fund-action="del" data-c="solarSales" data-id="${r.id}">حذف</button></td></tr>`).join('')||`<tr><td colspan="6">${empty('لا توجد مبيعات')}</td></tr>`}</tbody></table></div></div></div>`;
  if(tab==='fund') html+=`<section class="panel">${header('مصروفات صندوق العمارة','هذه المصروفات تخص الصندوق فقط.','<button class="btn primary" data-fund-action="add-fund-exp">+ إضافة مصروف</button>')}<div class="fund-lines"><div><span>رصيد البداية</span><b>${fundMoney(f.opening)}</b></div><div><span>صافي السولار الداخل للصندوق</span><b>${fundMoney(f.solarNet)}</b></div><div><span>إيرادات أخرى</span><b>${fundMoney(f.other)}</b></div><div><span>المصروفات</span><b>${fundMoney(f.expenses)}</b></div><div class="highlight"><span>الرصيد</span><b>${fundMoney(f.closing)}</b></div></div><div class="actions" style="margin-bottom:14px"><button class="btn soft" data-fund-action="edit-opening">تعديل الرصيد الافتتاحي</button></div><div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>النوع</th><th>الوصف</th><th>المبلغ</th><th>المستفيد</th><th></th></tr></thead><tbody>${fundRows('fundExpenses').filter(x=>String(x.date||'').startsWith(state.fundMonth)).sort((a,b)=>String(a.date).localeCompare(String(b.date))).map(r=>`<tr><td>${fmtDate(r.date)}</td><td>${safe(r.type||'')}</td><td>${safe(r.description||'')}</td><td>${fundMoney(r.amount)}</td><td>${safe(r.payee||'')}</td><td><button class="btn small ghost" data-fund-action="edit-fund-exp" data-id="${r.id}">تعديل</button><button class="btn small danger" data-fund-action="del" data-c="fundExpenses" data-id="${r.id}">حذف</button></td></tr>`).join('')||`<tr><td colspan="6">${empty('لا توجد مصروفات صندوق')}</td></tr>`}</tbody></table></div></section>`;
  if(tab==='other') html+=`<section class="panel">${header('إيرادات أخرى','حدد أين يدخل الإيراد: صندوق العمارة أو حساب خدمات الحارس.','<button class="btn primary" data-fund-action="add-income">+ إيراد جديد</button>')}<div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>النوع</th><th>المبلغ</th><th>المصدر</th><th>المكان</th><th></th></tr></thead><tbody>${fundRows('fundOtherIncome').filter(x=>String(x.date||'').startsWith(state.fundMonth)).map(r=>`<tr><td>${fmtDate(r.date)}</td><td>${safe(r.name||'')}</td><td>${fundMoney(r.amount)}</td><td>${safe(r.source||'')}</td><td>${r.destination==='guard'?'حساب الحارس':'صندوق العمارة'}</td><td><button class="btn small ghost" data-fund-action="edit-income" data-id="${r.id}">تعديل</button><button class="btn small danger" data-fund-action="del" data-c="fundOtherIncome" data-id="${r.id}">حذف</button></td></tr>`).join('')||`<tr><td colspan="6">${empty('لا توجد إيرادات أخرى')}</td></tr>`}</tbody></table></div></section>`;
  html+=`</section>`;
  $('#app').innerHTML=html;
  $('#fundMonthPicker').onchange=e=>{state.fundMonth=e.target.value;render();};
}
...
function render(){const fn={dashboard:renderDashboard,periods:renderPeriods,readings:renderReadings,energy:renderEnergy,costs:renderCosts,guard:renderGuard,contributions:renderContributions,subscribers:renderSubscribers,payments:renderPayments,debts:renderDebts,reports:renderReports,fund:renderFund,settings:renderSettings,guide:showGuide,historical:renderHistorical}[state.view]||renderDashboard;fn();}
async function navigate(view,periodId=null,force=false){if(!state.profile||state.profile.role==='pending'){renderPending();return;}state.view=view;if(periodId)state.periodId=periodId;setActiveNav();if(view==='fund') await loadFundData(force); else await loadData(force);render();$('#sidebar')?.classList.remove('open');}
...
function bindReadingInputs(pid){
  ...
  $('#saveReadings').onclick=async()=>{
    ...
    for(const tr of rows){
      ...
      const manual=priceInput.dataset.manual==='1'?num(priceInput.value):null; const price=manual!=null?manual:weekPrice;
      const consumption=cur!==''&&prev!==''?Math.max(0,num(cur)-num(prev))/1000:null;
      ops.push(b=>b.update(orgDoc('readings',tr.dataset.rid),{previousReading:prev===''?null:num(prev),currentReading:cur===''?null:num(cur),consumption,unitPrice:price,manualUnitPrice:manual,manualPrice:manual!=null,chargeAmount:consumption!=null?consumption*price:null,status:cur===''?'Pending':'Entered',updatedAt:serverTimestamp(),updatedBy:state.user.uid}));
    }
...
function totalSubscribersDebt(){return (state.data.subscribers||[]).filter(s=>s.active!==false).reduce((a,s)=>a+Math.max(0,num(subscriberRow(s).debt)),0);}
function renderSubscribers(){
  ...
  $('#app').innerHTML=`<section class="grid-2">
    <div class="panel">
      ...
      <div class="debt-summary-card"><span>إجمالي مديونية السكان الحالية</span><strong class="ltr">${money(totalSubscribersDebt())}</strong><small>يتحدث تلقائيًا مع تغيّر حسابات السكان.</small></div>
...
function showPaymentForm(id=null){
  ...
  $('#savePay').onclick=async()=>{
    ...
    if(existing){
      ...
      else {const lr=doc(orgCollection('ledger'));await setDoc(lr,{subscriberId,periodId,transactionType:'PAYMENT',credit:amount,debit:0,description:'دفعة',referenceId:id,paymentDate,createdAt:serverTimestamp(),createdBy:state.user.uid});}
    }else{
...
function renderGuard(){
  ...
  $('#app').innerHTML=`<section class="panel">
    <div class="panel-head"><div><h2>خدمة الحارس</h2><p>${guardEligible?'هذه الرسوم تُسجّل في أول أسبوع فقط من كل شهر.':'هذا الأسبوع ليس أول أسبوع من الشهر، لذلك لا تُضاف خدمة الحارس هنا.'}</p></div><button class="btn ghost" id="guardDeleteWeek">حذف خدمات الحارس لهذا الأسبوع</button></div>
  ...
  $('#saveDebt').onclick=async()=>{
    ...
    if(d){
      ...
      if(led)await updateDoc(orgDoc('ledger',led.id),{subscriberId,periodId:data.openingPeriodId||null,debit:amount-num(data.paidAmount||0),credit:0,description:data.description,date:data.date,isOpeningDebt:true,updatedAt:serverTimestamp()});
        ...
        await setDoc(lr,{subscriberId,periodId:data.openingPeriodId||null,transactionType:'DEBT',debit:amount-num(data.paidAmount||0),credit:0,description:data.description,referenceId:id,date:data.date,isOpeningDebt:true,createdAt:serverTimestamp(),createdBy:state.user.uid});
      }
    }else{
...
document.addEventListener('click',async e=>{const tab=e.target.closest('[data-fund-tab]');if(tab){state.fundTab=tab.dataset.fundTab;render();return;}const a=e.target.closest('[data-fund-action]');if(!a)return;try{const act=a.dataset.fundAction;if(act==='guard-settings')fundGuardMonthForm();else if(act==='save-guard')await fundSaveGuardPayments();else if(act==='add-clean')fundGuardExpenseForm();else if(act==='edit-clean')fundGuardExpenseForm(a.dataset.id);else if(act==='add-receipt')fundSolarReceiptForm();else if(act==='edit-receipt')fundSolarReceiptForm(a.dataset.id);else if(act==='add-sale')fundSolarSaleForm();else if(act==='edit-sale')fundSolarSaleForm(a.dataset.id);else if(act==='add-income')fundOtherIncomeForm();else if(act==='edit-income')fundOtherIncomeForm(a.dataset.id);else if(act==='add-fund-exp')fundExpenseForm();else if(act==='edit-fund-exp')fundExpenseForm(a.dataset.id);else if(act==='edit-opening')fundOpeningForm();else if(act==='del')await fundDeleteDoc(a.dataset.c,a.dataset.id,'السجل');}catch(err){console.error(err);toast(err?.message||'حدث خطأ في صندوق العمارة','error')}});
...
async function deleteDebt(id){if(!can('admin','manager')){toast('الحذف مخصص للمديرين','error');return;}const d=(state.data.debts||[]).find(x=>x.id===id);if(!d)return;if(!confirm(`حذف الدين بمبلغ ${money(d.amount)}؟`))return;const ops=[b=>b.delete(orgDoc('debts',id))];for(const l of (state.data.ledger||[]).filter(x=>x.referenceId===id&&x.transactionType==='DEBT'))ops.push(b=>b.delete(orgDoc('ledger',l.id)));await commitOps(ops);state.loaded=false;await loadData(true);toast('تم حذف الدين');renderDebts();}
...
async function deleteExternalWaterSummary(pid){if(!can('admin','manager')){toast('الحذف مخصص للمديرين','error');return;}const r=waterSummaryForPeriod(pid).find(x=>x.key==='external'||x.type==='external');if(!r)return;if(!confirm('حذف قراءة الخارجي لهذا الأسبوع؟'))return;await deleteDoc(orgDoc('waterSummary',r.id));removeLocal('waterSummary',r.id);toast('تم حذف قراءة الخارجي');renderReadings();}

async function deleteEnergyReading(id){if(!can('admin','manager')){toast('الحذف مخصص للمديرين','error');return;}if(!confirm('حذف قراءة الكهرباء؟'))return;await deleteDoc(orgDoc('energyReadings',id));removeLocal('energyReadings',id);toast('تم حذف قراءة الكهرباء');renderEnergy();}
