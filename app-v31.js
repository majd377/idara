...
const COLLECTIONS=['buildings','units','subscribers','meters','periods','readings','sources','energyReadings','costs','contributions','payments','ledger','members','waterSummary','seedDeletes','debts','fundGuardConfig','fundGuardPayments','fundGuardExpenses','solarReceipts','solarSales','fundOtherIncome','fundExpenses'];
...
// =========================
// V32 — إيرادات وصندوق العمارة (مستقل عن حسبة المياه)
// =========================
const FUND_COLLECTIONS=['fundGuardConfig','fundGuardPayments','fundGuardExpenses','solarReceipts','solarSales','fundOtherIncome','fundExpenses'];
function fundRows(c){return state.data[c]||[];}
function fundMonthKey(v){return String(v||'').slice(0,7);}
function fundSelectedMonth(){
  const p=selectedPeriod();
  return state.__fundMonth || fundMonthKey(p?.startDate) || new Date().toISOString().slice(0,7);
}
function fundMonthLabel(k){
  if(!k)return '—';
  const [y,m]=k.split('-');
  const names=['يناير','فبراير','مارس','أبريل','مايو','يونيو','يوليو','أغسطس','سبتمبر','أكتوبر','نوفمبر','ديسمبر'];
  return `${names[Math.max(0,Number(m)-1)]||m} ${y}`;
}
function fundConfig(){return fundRows('fundGuardConfig').find(x=>x.id==='settings')||{};}
function fundGuardPaymentsForMonth(m){return fundRows('fundGuardPayments').filter(x=>fundMonthKey(x.month||x.date||x.paymentDate)===m);}
function fundGuardExpensesForMonth(m){return fundRows('fundGuardExpenses').filter(x=>fundMonthKey(x.date||x.month)===m);}
function fundSolarReceiptsForMonth(m){return fundRows('solarReceipts').filter(x=>fundMonthKey(x.date)===m);}
function fundSolarSalesForMonth(m){return fundRows('solarSales').filter(x=>fundMonthKey(x.date)===m);}
function fundOtherIncomeForMonth(m){return fundRows('fundOtherIncome').filter(x=>fundMonthKey(x.date)===m);}
function fundExpensesForMonth(m){return fundRows('fundExpenses').filter(x=>fundMonthKey(x.date)===m);}
function fundGuardIncomeMonth(m){return fundGuardPaymentsForMonth(m).reduce((a,x)=>a+num(x.amountPaid),0);}
function fundGuardRequiredMonth(m){return fundGuardPaymentsForMonth(m).reduce((a,x)=>a+num(x.amountDue),0);}
function fundGuardCleaningMonth(m){return fundGuardExpensesForMonth(m).filter(x=>String(x.type||'')==='cleaning').reduce((a,x)=>a+num(x.amount),0);}
function fundGuardSalaryMonth(m){return fundGuardExpensesForMonth(m).filter(x=>String(x.type||'')==='salary').reduce((a,x)=>a+num(x.amount),0);}
function fundSolarSalesValueMonth(m){return fundSolarSalesForMonth(m).reduce((a,x)=>a+num(x.totalValue),0);}
function fundSolarTransportMonth(m){return fundSolarReceiptsForMonth(m).reduce((a,x)=>a+num(x.transportCost),0);}
function fundSolarPurchasedMonth(m){return fundSolarReceiptsForMonth(m).reduce((a,x)=>a+num(x.liters),0);}
function fundSolarSoldMonth(m){return fundSolarSalesForMonth(m).reduce((a,x)=>a+num(x.liters),0);}
function fundOtherToFundMonth(m){return fundOtherIncomeForMonth(m).filter(x=>x.destination==='fund').reduce((a,x)=>a+num(x.amount),0);}
function fundOtherToGuardMonth(m){return fundOtherIncomeForMonth(m).filter(x=>x.destination==='guard').reduce((a,x)=>a+num(x.amount),0);}
function fundFundExpensesMonth(m){return fundExpensesForMonth(m).reduce((a,x)=>a+num(x.amount),0);}
function fundNetGuardMonth(m){return fundGuardIncomeMonth(m)+fundOtherToGuardMonth(m)-fundGuardSalaryMonth(m)-fundGuardCleaningMonth(m);}
function fundNetSolarMonth(m){return fundSolarSalesValueMonth(m)-fundSolarTransportMonth(m);}
function fundMonthlyKeys(){
  const set=new Set();
  [...fundRows('fundGuardPayments'),...fundRows('fundGuardExpenses'),...fundRows('solarReceipts'),...fundRows('solarSales'),...fundRows('fundOtherIncome'),...fundRows('fundExpenses')].forEach(x=>{const k=fundMonthKey(x.month||x.date||x.paymentDate);if(/^\d{4}-\d{2}$/.test(k))set.add(k);});
  const p=selectedPeriod();if(p?.startDate)set.add(fundMonthKey(p.startDate));
  const c=fundConfig();if(c.startingBalanceMonth)set.add(c.startingBalanceMonth);
  return [...set].sort();
}
function fundOpeningBalance(month){
  const keys=fundMonthlyKeys().filter(k=>k<month);
  let total=0;
  for(const k of keys) total += fundNetSolarMonth(k)+fundOtherToFundMonth(k)-fundFundExpensesMonth(k);
  return total;
}
function fundCurrentBalanceThrough(month){
  const keys=fundMonthlyKeys().filter(k=>k<=month);
  return keys.reduce((a,k)=>a+fundNetSolarMonth(k)+fundOtherToFundMonth(k)-fundFundExpensesMonth(k),0);
}
function fundMonthOptions(selected){
  const keys=fundMonthlyKeys();
  const base=selected||new Date().toISOString().slice(0,7);
  if(!keys.includes(base))keys.push(base);
  keys.sort();
  return keys.map(k=>`<option value="${k}" ${k===base?'selected':''}>${safe(fundMonthLabel(k))}</option>`).join('');
}
...
function fundActions(collection,id){
  ...
  return `<div class="row-actions">${edit}${del}</div>`;
}
function fundGuardStatus(p){
  const due=num(p.amountDue), paid=num(p.amountPaid);
  if(paid<=0)return 'غير مدفوع'; if(paid+1e-9<due)return 'مدفوع جزئيًا'; return 'مدفوع';
}
function renderFund(){
  setTitle('إيرادات وصندوق العمارة','صفحة مستقلة — كل الإيرادات تدخلها أنت يدويًا، ولا تدخل في حسبة المياه أو الكهرباء.');
  if(!can('admin','manager','accountant')){ $('#app').innerHTML=`<section class="panel">${empty('هذه الصفحة للإدارة','الصندوق والإيرادات مخصصة للإدارة والحسابات.')}</section>`;return; }
  const month=fundSelectedMonth();
  const cfg=fundConfig();
  const guardPayments=fundGuardPaymentsForMonth(month);
  const guardExpenses=fundGuardExpensesForMonth(month);
  const sales=fundSolarSalesForMonth(month), receipts=fundSolarReceiptsForMonth(month), other=fundOtherIncomeForMonth(month), expenses=fundExpensesForMonth(month);
  const guardIncome=fundGuardIncomeMonth(month), guardReq=fundGuardRequiredMonth(month), salary=fundGuardSalaryMonth(month), clean=fundGuardCleaningMonth(month), guardOther=fundOtherToGuardMonth(month), guardNet=fundNetGuardMonth(month);
  const solarSales=fundSolarSalesValueMonth(month), transport=fundSolarTransportMonth(month), netSolar=fundNetSolarMonth(month), bought=fundSolarPurchasedMonth(month), sold=fundSolarSoldMonth(month);
  const opening=fundOpeningBalance(month), fundOther=fundOtherToFundMonth(month), fundOut=fundFundExpensesMonth(month), closing=opening+netSolar+fundOther-fundOut;
  const balanceLiters=Math.max(0,fundMonthlyKeys().filter(k=>k<=month).reduce((a,k)=>a+fundSolarPurchasedMonth(k)-fundSolarSoldMonth(k),0));
  const members=(state.data.subscribers||[]).filter(s=>s.active!==false&&s.type!=='خارجي').sort((a,b)=>String(a.code||'').localeCompare(String(b.code||''),undefined,{numeric:true}));
  const bySid=new Map(guardPayments.map(x=>[x.subscriberId,x]));
  const duePerPerson=cfg.perPersonAmount==null?30:num(cfg.perPersonAmount);
  $('#app').innerHTML=`
    <section class="hero"><div><span class="guide-badge">حساب مستقل</span><h2>إيرادات وصندوق العمارة</h2><p>كل دخل هنا يُسجَّل يدويًا. البرنامج يجمع ويحسب فقط، ولا ينشئ إيرادًا من عدد السكان من تلقاء نفسه.</p></div><div class="hero-actions"><div class="period-picker"><label>الشهر</label><select id="fundMonth" class="period-select">${fundMonthOptions(month)}</select></div></div></section>
    <section class="cards-grid fund-cards">
      <div class="stat-card"><span>رصيد الصندوق حتى ${safe(fundMonthLabel(month))}</span><b>${money(closing)}</b></div>
      <div class="stat-card"><span>صافي خدمات الحارس</span><b>${money(guardNet)}</b></div>
      <div class="stat-card"><span>صافي السولار</span><b>${money(netSolar)}</b></div>
      <div class="stat-card"><span>كمية السولار المتبقية</span><b>${fmt(balanceLiters,3)} لتر</b></div>
    </section>

    <section class="panel fund-section"><div class="panel-head"><div><h2>خدمة الحارس</h2><p>الإيراد الفعلي يُدخل يدويًا لكل عملية دفع. المبلغ على الشخص لا يولّد إيرادًا تلقائيًا.</p></div><div class="actions"><button class="btn primary" id="addGuardPayment">+ تسجيل دفعة حارس</button><button class="btn ghost" id="addGuardExpense">+ مصروف حارس</button></div></div>
      <div class="money-grid"><div class="money-card"><small>إجمالي المطلوب المسجل</small><b>${money(guardReq)}</b></div><div class="money-card"><small>إجمالي المحصل</small><b>${money(guardIncome)}</b></div><div class="money-card"><small>راتب الحارس</small><b>${money(salary)}</b></div><div class="money-card"><small>النظافة</small><b>${money(clean)}</b></div></div>
      <div class="form-grid" style="margin-top:12px"><div class="field"><label>المبلغ المقترح على الشخص (للمتابعة فقط)</label><input id="fundPerPerson" type="number" step="1" min="0" value="${safe(duePerPerson)}"></div><div class="field"><label>رصيد خدمة الحارس</label><input value="${safe(money(guardNet))}" disabled></div></div>
      <div class="table-wrap"><table class="table"><thead><tr><th>الكود</th><th>الاسم</th><th>المطلوب</th><th>المدفوع</th><th>المتبقي</th><th>الحالة</th></tr></thead><tbody>${members.map(s=>{const r=bySid.get(s.id)||{};const due=r.amountDue!=null?r.amountDue:0;const paid=r.amountPaid!=null?r.amountPaid:0;return `<tr><td><span class="code">${safe(s.code)}</span></td><td>${safe(s.name)}</td><td>${money(due)}</td><td>${money(paid)}</td><td>${money(Math.max(0,due-paid))}</td><td>${safe(fundGuardStatus({amountDue:due,amountPaid:paid}))}</td></tr>`}).join('')||`<tr><td colspan="6">لا توجد بيانات دفع مسجلة لهذا الشهر.</td></tr>`}</tbody></table></div>
      <div class="section-note">الراتب والنظافة يُدخلان من مصاريف الحارس. الإيراد المحصل هو مجموع الدفعات التي سجلتها أنت.</div>
      <div class="table-wrap" style="margin-top:12px"><table class="table"><thead><tr><th>التاريخ</th><th>نوع المصروف</th><th>البيان</th><th>المبلغ</th><th>الإجراءات</th></tr></thead><tbody>${guardExpenses.map(x=>`<tr><td>${safe(fmtDate(x.date))}</td><td>${x.type==='salary'?'راتب الحارس':'نظافة / أدوات'}</td><td>${safe(x.description||'—')}</td><td>${money(x.amount)}</td><td>${fundActions('fundGuardExpenses',x.id)}</td></tr>`).join('')||`<tr><td colspan="5">لا توجد مصروفات مسجلة.</td></tr>`}</tbody></table></div>
    </section>

    <section class="panel fund-section"><div class="panel-head"><div><h2>السولار</h2><p>تدخل أنت كل عملية استلام وكل عملية بيع. صافي المبيعات بعد النقل يذهب للصندوق.</p></div><div class="actions"><button class="btn primary" id="addSolarReceipt">+ استلام سولار</button><button class="btn ghost" id="addSolarSale">+ بيع سولار</button></div></div>
      <div class="money-grid"><div class="money-card"><small>المستلم</small><b>${fmt(bought,3)} لتر</b></div><div class="money-card"><small>المباع</small><b>${fmt(sold,3)} لتر</b></div><div class="money-card"><small>قيمة المبيعات</small><b>${money(solarSales)}</b></div><div class="money-card"><small>نقل السولار</small><b>${money(transport)}</b></div><div class="money-card"><small>الصافي للصندوق</small><b>${money(netSolar)}</b></div></div>
      <div class="sub-section"><div class="section-head-inline"><div><h3>عمليات الاستلام</h3></div></div><div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>اللترات</th><th>المصدر</th><th>النقل</th><th>ملاحظات</th><th>الإجراءات</th></tr></thead><tbody>${receipts.map(x=>`<tr><td>${safe(fmtDate(x.date))}</td><td>${fmt(x.liters,3)}</td><td>${safe(x.source||'—')}</td><td>${money(x.transportCost)}</td><td>${safe(x.notes||'—')}</td><td>${fundActions('solarReceipts',x.id)}</td></tr>`).join('')||`<tr><td colspan="6">لا توجد عمليات استلام لهذا الشهر.</td></tr>`}</tbody></table></div></div>
      <div class="sub-section"><div class="section-head-inline"><div><h3>عمليات البيع</h3></div></div><div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>اللترات</th><th>سعر اللتر</th><th>الإجمالي</th><th>المشتري</th><th>الإجراءات</th></tr></thead><tbody>${sales.map(x=>`<tr><td>${safe(fmtDate(x.date))}</td><td>${fmt(x.liters,3)}</td><td>${money(x.pricePerLiter)}</td><td>${money(x.totalValue)}</td><td>${safe(x.buyer||'—')}</td><td>${fundActions('solarSales',x.id)}</td></tr>`).join('')||`<tr><td colspan="6">لا توجد عمليات بيع لهذا الشهر.</td></tr>`}</tbody></table></div></div>
    </section>

    <section class="panel fund-section"><div class="panel-head"><div><h2>الإيرادات الأخرى</h2><p>أدخل أي إيراد يدويًا وحدد أين يُضاف.</p></div><button class="btn primary" id="addOtherIncome">+ إضافة إيراد</button></div>
      <div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>البيان</th><th>المصدر</th><th>المبلغ</th><th>يضاف إلى</th><th>الإجراءات</th></tr></thead><tbody>${other.map(x=>`<tr><td>${safe(fmtDate(x.date))}</td><td>${safe(x.type||'—')}</td><td>${safe(x.source||'—')}</td><td>${money(x.amount)}</td><td>${x.destination==='fund'?'صندوق العمارة':'حساب الحارس'}</td><td>${fundActions('fundOtherIncome',x.id)}</td></tr>`).join('')||`<tr><td colspan="6">لا توجد إيرادات أخرى.</td></tr>`}</tbody></table></div>
    </section>

    <section class="panel fund-section"><div class="panel-head"><div><h2>مصروفات صندوق العمارة</h2><p>هذه المصروفات تخص الصندوق ولا تدخل في حساب المياه.</p></div><button class="btn primary" id="addFundExpense">+ تسجيل مصروف صندوق</button></div>
      <div class="money-grid"><div class="money-card"><small>رصيد بداية الشهر</small><b>${money(opening)}</b></div><div class="money-card"><small>داخل من السولار</small><b>${money(netSolar)}</b></div><div class="money-card"><small>إيرادات أخرى للصندوق</small><b>${money(fundOther)}</b></div><div class="money-card"><small>مصروفات الصندوق</small><b>${money(fundOut)}</b></div><div class="money-card"><small>رصيد نهاية الشهر</small><b>${money(closing)}</b></div></div>
      <div class="table-wrap"><table class="table"><thead><tr><th>التاريخ</th><th>نوع المصروف</th><th>الوصف</th><th>المستفيد</th><th>المبلغ</th><th>الإجراءات</th></tr></thead><tbody>${expenses.map(x=>`<tr><td>${safe(fmtDate(x.date))}</td><td>${safe(x.type||'—')}</td><td>${safe(x.description||'—')}</td><td>${safe(x.beneficiary||'—')}</td><td>${money(x.amount)}</td><td>${fundActions('fundExpenses',x.id)}</td></tr>`).join('')||`<tr><td colspan="6">لا توجد مصروفات للصندوق.</td></tr>`}</tbody></table></div>
    </section>`;
  $('#fundMonth').onchange=e=>{state.__fundMonth=e.target.value;renderFund();};
  $('#fundPerPerson').onchange=async()=>{if(!can('admin','manager'))return;const val=num($('#fundPerPerson').value);await fundUpsert('fundGuardConfig','settings',{perPersonAmount:val});toast('تم حفظ القيمة المقترحة على الشخص');};
  $('#addGuardPayment').onclick=()=>showFundGuardPaymentForm(month);
  $('#addGuardExpense').onclick=()=>showFundGuardExpenseForm(month);
  $('#addSolarReceipt').onclick=()=>showFundSolarReceiptForm(month);
  $('#addSolarSale').onclick=()=>showFundSolarSaleForm(month);
  $('#addOtherIncome').onclick=()=>showFundOtherIncomeForm(month);
  $('#addFundExpense').onclick=()=>showFundExpenseForm(month);
  $$('[data-fund-edit]').forEach(b=>b.onclick=()=>{const [c,id]=b.dataset.fundEdit.split(':');showFundFormFor(c,id,month);});
  $$('[data-fund-delete]').forEach(b=>b.onclick=async()=>{const [c,id]=b.dataset.fundDelete.split(':');await deleteFundRecord(c,id);});
}
function showFundGuardPaymentForm(month,id=null){
  const row=id?fundRows('fundGuardPayments').find(x=>x.id===id):null;
  const subs=(state.data.subscribers||[]).filter(s=>s.active!==false&&s.type!=='خارجي').sort((a,b)=>String(a.code||'').localeCompare(String(b.code||''),undefined,{numeric:true}));
  openModal(`<h2>${row?'تعديل دفعة حارس':'تسجيل دفعة حارس'}</h2><div class="form-grid"><div class="field"><label>الساكن</label><select id="fgpSub" ${row?'disabled':''}>${subs.map(s=>`<option value="${s.id}" ${s.id===row?.subscriberId?'selected':''}>${safe(s.code)} — ${safe(s.name)}</option>`).join('')}</select></div><div class="field"><label>التاريخ</label><input id="fgpDate" type="date" value="${safe(row?.date||new Date().toISOString().slice(0,10))}"></div><div class="field"><label>المبلغ المطلوب (يدوي)</label><input id="fgpDue" type="number" min="0" step="1" value="${row?.amountDue??fundConfig().perPersonAmount??30}"></div><div class="field"><label>المبلغ المدفوع</label><input id="fgpPaid" type="number" min="0" step="1" value="${row?.amountPaid??''}"></div><div class="field full"><label>ملاحظة</label><input id="fgpNote" value="${safe(row?.note||'')}"></div></div><div class="actions"><button class="btn primary" id="fgpSave">حفظ</button><button class="btn ghost" id="fgpCancel">إلغاء</button></div>`);
  $('#fgpCancel').onclick=closeModal;$('#fgpSave').onclick=async()=>{const data={subscriberId:$('#fgpSub').value,month,date:$('#fgpDate').value,amountDue:num($('#fgpDue').value),amountPaid:num($('#fgpPaid').value),note:$('#fgpNote').value.trim()};if(data.amountPaid<0||data.amountDue<0){toast('المبلغ غير صحيح','error');return;}await fundUpsert('fundGuardPayments',id,data);closeModal();renderFund();toast(row?'تم تعديل الدفعة':'تم تسجيل الدفعة');};
}
function showFundGuardExpenseForm(month,id=null){
  const row=id?fundRows('fundGuardExpenses').find(x=>x.id===id):null;
  openModal(`<h2>${row?'تعديل مصروف حارس':'إضافة مصروف حارس'}</h2><div class="form-grid"><div class="field"><label>نوع المصروف</label><select id="fgeType"><option value="salary" ${row?.type==='salary'?'selected':''}>راتب الحارس</option><option value="cleaning" ${row?.type==='cleaning'?'selected':''}>نظافة / أدوات</option></select></div><div class="field"><label>التاريخ</label><input id="fgeDate" type="date" value="${safe(row?.date||new Date().toISOString().slice(0,10))}"></div><div class="field"><label>المبلغ</label><input id="fgeAmount" type="number" min="0" step="1" value="${row?.amount??''}"></div><div class="field full"><label>البيان</label><input id="fgeDesc" value="${safe(row?.description||'')}"></div></div><div class="actions"><button class="btn primary" id="fgeSave">حفظ</button><button class="btn ghost" id="fgeCancel">إلغاء</button></div>`);
  $('#fgeCancel').onclick=closeModal;$('#fgeSave').onclick=async()=>{const data={month:fundMonthKey($('#fgeDate').value),date:$('#fgeDate').value,type:$('#fgeType').value,amount:num($('#fgeAmount').value),description:$('#fgeDesc').value.trim()};if(data.amount<0){toast('المبلغ غير صحيح','error');return;}await fundUpsert('fundGuardExpenses',id,data);closeModal();renderFund();toast(row?'تم تعديل المصروف':'تم تسجيل المصروف');};
}
function showFundSolarReceiptForm(month,id=null){
  const row=id?fundRows('solarReceipts').find(x=>x.id===id):null;
  openModal(`<h2>${row?'تعديل استلام سولار':'تسجيل استلام سولار'}</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="fsrDate" type="date" value="${safe(row?.date||new Date().toISOString().slice(0,10))}"></div><div class="field"><label>كمية السولار (لتر)</label><input id="fsrLiters" type="number" min="0" step="0.001" value="${row?.liters??''}"></div><div class="field"><label>مصدر السولار</label><input id="fsrSource" value="${safe(row?.source||'البلدية')}"></div><div class="field"><label>تكلفة النقل</label><input id="fsrTransport" type="number" min="10" step="1" value="${row?.transportCost??10}"></div><div class="field full"><label>ملاحظات</label><input id="fsrNotes" value="${safe(row?.notes||'')}"></div></div><div class="actions"><button class="btn primary" id="fsrSave">حفظ</button><button class="btn ghost" id="fsrCancel">إلغاء</button></div>`);
  $('#fsrCancel').onclick=closeModal;$('#fsrSave').onclick=async()=>{const transport=num($('#fsrTransport').value);if(transport<10){toast('تكلفة النقل لا تقل عن 10 ₪','error');return;}const data={date:$('#fsrDate').value,month:fundMonthKey($('#fsrDate').value),liters:num($('#fsrLiters').value),source:$('#fsrSource').value.trim(),transportCost:transport,notes:$('#fsrNotes').value.trim()};if(data.liters<0){toast('الكمية غير صحيحة','error');return;}await fundUpsert('solarReceipts',id,data);closeModal();renderFund();toast(row?'تم تعديل الاستلام':'تم تسجيل الاستلام');};
}
function showFundSolarSaleForm(month,id=null){
  const row=id?fundRows('solarSales').find(x=>x.id===id):null;
  openModal(`<h2>${row?'تعديل بيع سولار':'تسجيل بيع سولار'}</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="fssDate" type="date" value="${safe(row?.date||new Date().toISOString().slice(0,10))}"></div><div class="field"><label>اللترات المباعة</label><input id="fssLiters" type="number" min="0" step="0.001" value="${row?.liters??''}"></div><div class="field"><label>سعر اللتر</label><input id="fssPrice" type="number" min="0" step="0.01" value="${row?.pricePerLiter??''}"></div><div class="field"><label>إجمالي البيع المباشر (اختياري)</label><input id="fssTotal" type="number" min="0" step="1" value="${row?.manualTotal??''}"></div><div class="field full"><label>اسم المشتري (اختياري)</label><input id="fssBuyer" value="${safe(row?.buyer||'')}"></div><div class="field full"><label>ملاحظات</label><input id="fssNotes" value="${safe(row?.notes||'')}"></div></div><div class="actions"><button class="btn primary" id="fssSave">حفظ</button><button class="btn ghost" id="fssCancel">إلغاء</button></div>`);
  $('#fssCancel').onclick=closeModal;$('#fssSave').onclick=async()=>{const liters=num($('#fssLiters').value),price=num($('#fssPrice').value),manual=$('#fssTotal').value.trim()===''?null:num($('#fssTotal').value);if(liters<0||price<0||(manual!=null&&manual<0)){toast('تحقق من القيم','error');return;}const total=manual!=null?manual:liters*price;const data={date:$('#fssDate').value,month:fundMonthKey($('#fssDate').value),liters,pricePerLiter:price,manualTotal:manual,totalValue:total,buyer:$('#fssBuyer').value.trim(),notes:$('#fssNotes').value.trim()};await fundUpsert('solarSales',id,data);closeModal();renderFund();toast(row?'تم تعديل البيع':'تم تسجيل البيع');};
}
function showFundOtherIncomeForm(month,id=null){
  const row=id?fundRows('fundOtherIncome').find(x=>x.id===id):null;
  openModal(`<h2>${row?'تعديل إيراد آخر':'إضافة إيراد آخر'}</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="foiDate" type="date" value="${safe(row?.date||new Date().toISOString().slice(0,10))}"></div><div class="field"><label>اسم/نوع الإيراد</label><input id="foiType" value="${safe(row?.type||'')}"></div><div class="field"><label>المبلغ</label><input id="foiAmount" type="number" min="0" step="1" value="${row?.amount??''}"></div><div class="field"><label>المصدر</label><input id="foiSource" value="${safe(row?.source||'')}"></div><div class="field"><label>يضاف إلى</label><select id="foiDest"><option value="fund" ${row?.destination==='fund'?'selected':''}>صندوق العمارة</option><option value="guard" ${row?.destination==='guard'?'selected':''}>حساب خدمات الحارس</option></select></div><div class="field full"><label>ملاحظات</label><input id="foiNotes" value="${safe(row?.notes||'')}"></div></div><div class="actions"><button class="btn primary" id="foiSave">حفظ</button><button class="btn ghost" id="foiCancel">إلغاء</button></div>`);
  $('#foiCancel').onclick=closeModal;$('#foiSave').onclick=async()=>{const data={date:$('#foiDate').value,month:fundMonthKey($('#foiDate').value),type:$('#foiType').value.trim(),amount:num($('#foiAmount').value),source:$('#foiSource').value.trim(),destination:$('#foiDest').value,notes:$('#foiNotes').value.trim()};if(data.amount<0){toast('المبلغ غير صحيح','error');return;}await fundUpsert('fundOtherIncome',id,data);closeModal();renderFund();toast(row?'تم تعديل الإيراد':'تم تسجيل الإيراد');};
}
function showFundExpenseForm(month,id=null){
  const row=id?fundRows('fundExpenses').find(x=>x.id===id):null;
  openModal(`<h2>${row?'تعديل مصروف صندوق':'تسجيل مصروف صندوق'}</h2><div class="form-grid"><div class="field"><label>التاريخ</label><input id="ffeDate" type="date" value="${safe(row?.date||new Date().toISOString().slice(0,10))}"></div><div class="field"><label>نوع المصروف</label><input id="ffeType" value="${safe(row?.type||'') }" placeholder="مثال: صيانة مضخة"></div><div class="field"><label>المبلغ</label><input id="ffeAmount" type="number" min="0" step="1" value="${row?.amount??''}"></div><div class="field"><label>المستفيد</label><input id="ffeBeneficiary" value="${safe(row?.beneficiary||'')}"></div><div class="field full"><label>وصف المصروف</label><input id="ffeDesc" value="${safe(row?.description||'')}"></div></div><div class="actions"><button class="btn primary" id="ffeSave">حفظ</button><button class="btn ghost" id="ffeCancel">إلغاء</button></div>`);
  $('#ffeCancel').onclick=closeModal;$('#ffeSave').onclick=async()=>{const data={date:$('#ffeDate').value,month:fundMonthKey($('#ffeDate').value),type:$('#ffeType').value.trim(),amount:num($('#ffeAmount').value),beneficiary:$('#ffeBeneficiary').value.trim(),description:$('#ffeDesc').value.trim()};if(data.amount<0){toast('المبلغ غير صحيح','error');return;}await fundUpsert('fundExpenses',id,data);closeModal();renderFund();toast(row?'تم تعديل المصروف':'تم تسجيل مصروف الصندوق');};
}
function showFundFormFor(collection,id,month){
  if(collection==='fundGuardPayments')showFundGuardPaymentForm(month,id);
  else if(collection==='fundGuardExpenses')showFundGuardExpenseForm(month,id);
  else if(collection==='solarReceipts')showFundSolarReceiptForm(month,id);
  else if(collection==='solarSales')showFundSolarSaleForm(month,id);
  else if(collection==='fundOtherIncome')showFundOtherIncomeForm(month,id);
  else if(collection==='fundExpenses')showFundExpenseForm(month,id);
}
async function deleteFundRecord(collection,id){
  if(!can('admin','manager')){toast('الحذف مخصص للمديرين','error');return;}
  const labels={fundGuardPayments:'دفعة الحارس',fundGuardExpenses:'مصروف الحارس',solarReceipts:'عملية استلام السولار',solarSales:'عملية بيع السولار',fundOtherIncome:'الإيراد',fundExpenses:'مصروف الصندوق'};
  if(!confirm(`حذف ${labels[collection]||'السجل'}؟`))return;
  await fundRemove(collection,id);toast('تم الحذف');renderFund();
}
