...
function renderDashboard(){
  setTitle('الرئيسية','من هنا تبدأ شغلك اليومي، ومعك ملخص سريع للصندوق والسكان.');const periods=latestPeriods(),latest=periods[0];const subs=(state.data.subscribers||[]).filter(s=>s.active!==false&&s.type!=='خارجي');const totals=latest?currentTotals(latest.id):null;const fundRev=fundRevenueTotal(),fundOut=fundWithdrawalTotal(),fundBal=fundRev-fundOut,solarQty=solarAvailableQty();
  $('#app').innerHTML=`<section class="welcome"><div class="kicker">عمارة الأمين • الإدارة اليومية</div><h2>أهلاً ${safe(state.user?.displayName?.split(' ')[0]||'بك')} 👋</h2><p>تابع الأسبوع والحسابات، ومعها ملخص خفيف للصندوق. الإيرادات والسحب منفصلة عن حساب السكان.</p><div class="welcome-actions"><button class="btn primary" id="dashNewWeek">+ افتح أسبوعًا</button><button class="btn ghost" id="dashReadings">إدخال قراءات الماء</button><button class="btn soft" id="dashFund">فتح الايرادات والصندوق</button></div></section>
  ...
  $('#dashNewWeek').onclick=showPeriodForm;$('#dashReadings').onclick=()=>latest?navigate('readings',latest.id):showPeriodForm;$('#dashFund').onclick=()=>navigate('fund');if(latest)$('#dashLatest').onclick=()=>navigate('periods',latest.id);
}
...
function showGuide(){
  const steps=[
    ...
    {title:'2) الأسابيع',text:'افتح أسبوعًا وحدد تاريخ البداية والنهاية، ثم تابع حساب الكهرباء والمياه وسعر الكوب.',go:'periods'},
    ...
    {title:'10) السكان',text:'إدارة بيانات السكان والوحدات، ومراجعة مديونية كل ساكن ومجموع المديونية في أسفل الصفحة.',go:'subscribers'},
    {title:'11) إضافة دفعة سولار',text:'كل مرة تضيف سولار سجل دفعة مستقلة بتاريخها وكمّيتها وبيانها. لا يوجد رصيد يدوي يُكتب.',go:'fund'},
    {title:'12) بيع السولار',text:'عند البيع أدخل اللترات وسعر اللتر والتاريخ. تُخصم الكمية من المتاح، وتُحسب قيمة البيع تلقائيًا.',go:'fund'},
    {title:'13) الإيرادات',text:'إضافة الإيراد لها: إيرادات سولار، إيرادات من خدمات الحارس، والإيرادات الأخرى. كل الإيرادات تذهب للصندوق فقط.',go:'fund'},
    {title:'14) سحب من الصندوق',text:'استخدمه عندما تدفع العمارة من الصندوق بدل تحميل المبلغ على السكان. السحب ينقص رصيد الصندوق فقط.',go:'fund'},
    {title:'15) التقارير',text:'التقارير للتصدير والحساب. وفي صفحة الإيرادات توجد طباعة منفصلة لكل جدول وملخص.',go:'reports'},
    {title:'16) البيانات التاريخية',text:'للوصول إلى المعلومات التاريخية المحفوظة في النظام.',go:'historical'},
    {title:'17) الإعدادات والصلاحيات',text:'لإدارة المستخدمين والصلاحيات وإعدادات النظام.',go:'settings'}
...
onAuthStateChanged(auth,async user=>{state.user=user;$('#boot')?.classList.add('hidden');if(!user){$('#auth-screen')?.classList.remove('hidden');$('#app-shell')?.classList.add('hidden');return;}$('#auth-screen')?.classList.add('hidden');$('#app-shell')?.classList.remove('hidden');$('#userName').textContent=user.displayName||user.email||'المستخدم';$('#userAvatar').textContent=(user.displayName||user.email||'م').slice(0,1);$('#userRole').textContent='جارٍ التحقق…';try{state.profile=await ensureProfile();$('#userRole').textContent=roleName(state.profile.role);if(state.profile.role==='pending'){renderPending();return;}await loadData(true);await ensureSolarBatchMigration();await ensureDefaults();await navigate('dashboard');}catch(e){console.error(e);$('#app').innerHTML=`<section class="panel" style="max-width:820px;margin:40px auto"><h2>تعذر تحميل البيانات</h2><p class="muted">${safe(e?.message||'تحقق من Firestore Rules وAuthorized Domains وإعدادات Firebase.')}</p></section>`;}});
